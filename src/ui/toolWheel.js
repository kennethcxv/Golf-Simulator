...
export function toolShortcutIndex(entries, key) {
  ...
  return entries.findIndex((entry) => String(entry.shortcut || '').toLowerCase() === normalized);
}

export function nextAvailableToolIndex(entries, selectedIndex, direction = 1) {
  const available = entries
    .map((entry, index) => entry.available !== false ? index : -1)
    .filter((index) => index >= 0);
  if (!available.length) return -1;
  const at = available.indexOf(selectedIndex);
  const origin = at >= 0 ? at : 0;
  return available[(origin + (direction < 0 ? -1 : 1) + available.length) % available.length];
}

export function makeToolWheel({ onSelect, onClose, onPause, onSwap, onUnavailable, audio } = {}) {
  ...
  const root = el('div', { class: 'tool-wheel', style: 'display:none' },
    ...
    el('div', { class: 'tool-wheel-panel', role: 'dialog', 'aria-modal': 'true', 'aria-label': 'Tool belt', tabindex: '-1' },
      ...
      el('div', { class: 'tool-wheel-center' }, selectedName, selectedDetail,
        el('div', { class: 'tool-wheel-help', text: 'Wheel, letter, number, or arrows · Enter equips · Esc closes · Q swaps back' })),
    ),
  );
  let entries = [];
  let selectedIndex = 0;
  ...
  function onKeyDown(event) {
    ...
    const shortcutIndex = toolShortcutIndex(entries, event.key);
    if (shortcutIndex >= 0) {
      event.stopPropagation();
      event.preventDefault();
  ...
  root.addEventListener('wheel', (event) => {
    ...
    event.preventDefault();
    event.stopPropagation();
    wheelRemainder += event.deltaY || event.deltaX;
    if (Math.abs(wheelRemainder) < 18) return;
    const next = nextAvailableToolIndex(entries, selectedIndex, Math.sign(wheelRemainder));
    wheelRemainder = 0;
    if (next < 0) return;
    highlight(next, { focus: true });
    audio?.uiTick?.();
  }, { passive: false });
  ...
  function show(nextEntries, currentId = null) {
    ...
    ring.replaceChildren(...entries.map((entry, index) => el('button', {
      ...
      'aria-label': entry.available === false
        ? `${entry.label}, ${entry.shortcut || index + 1}, unavailable: ${entry.reason || 'locked'}`
        : `${entry.label}, ${entry.shortcut || index + 1}`,
    ...
    el('span', { class: 'tool-wheel-icon', 'aria-hidden': 'true', text: entry.shortcut || entry.icon || '◇' }),
