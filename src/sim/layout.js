...
import {
  BACKDOOR_CLEARWAY, COUNTER, DOOR_CLEARWAY, FIXTURES, FRONT_DESK,
  FRONT_DESK_COLLIDERS, INTERIOR, LOUNGE, OFFICE, PARTITIONS, PLAYER_DIAM,
  SHELL, STAFF_CORRIDOR_MIN, STOCKROOM,
  fixtureBrowsePoint, fixtureCollisionRects, fixtureRect, queueSlot,
...
function migrateLayoutAlias(layout, legacyId, canonicalId) {
  if (layout.moved[legacyId] && !layout.moved[canonicalId]) {
    layout.moved[canonicalId] = clone(layout.moved[legacyId]);
  }
  delete layout.moved[legacyId];

  layout.stored = [...new Set(layout.stored.map((id) => (
    id === legacyId ? canonicalId : id
  )))];

  if (layout.objects[legacyId] && !layout.objects[canonicalId]) {
    const record = clone(layout.objects[legacyId]);
    if (record.assetId === `generated-fixture:${legacyId}`) {
      record.assetId = `generated-fixture:${canonicalId}`;
    }
    layout.objects[canonicalId] = record;
  }
  delete layout.objects[legacyId];

  // A short-lived furnished-layout build serialized the authored fixture as
  // an `extra`. Fold that pose back into the canonical fixture rather than
  // leaving two stock authorities in migrated saves.
  const legacyExtra = layout.extra.find((entry) => entry?.id === legacyId);
  if (legacyExtra && !layout.objects[canonicalId] && !layout.moved[canonicalId]) {
    layout.moved[canonicalId] = {
      x: legacyExtra.x,
      z: legacyExtra.z,
      ry: legacyExtra.ry || 0,
    };
  }
  layout.extra = layout.extra.filter((entry) => entry?.id !== legacyId);
}
...
export function ensureLayout(state) {
  ...
  // Idempotent bridge for the intermediate Pine Hills fixture spelling. The
  // stable logical ID remains `snackrack`; `snack_rack` is only the GLB name.
  migrateLayoutAlias(layout, 'snack_rack', 'snackrack');
...
export function placedObjects(state) {
  ...
  for (const meta of PLACEABLES) {
    if (meta.fixture && !fixtureIsInstalled(state, meta.id)) continue;
...
// Compatibility name used by the physical shop-tier and save/load suites.
// placedFixtures already applies the progression installation contract.
export const activeFixtures = (state, options = {}) => placedFixtures(state, options);
...
const STATIC_OBSTACLES = [
  { id: 'office-desk-legacy', facility: 'officeDesk', label: 'office desk', x: OFFICE.desk.x, z: OFFICE.desk.z, width: 0.95, depth: 1.9, height: 1.0, ry: 0 },
  { id: 'front-desk-chair', facility: 'officeChair', label: 'front-desk reception chair', x: FRONT_DESK.staffChair.x, z: FRONT_DESK.staffChair.z, width: 0.70, depth: 0.70, height: 1.1, ry: FRONT_DESK.staffChair.ry },
...
const campaignFacilityVisible = (state, id) => (
  !state?.campaign?.enabled || state.shop?.reno?.facilities?.[id] === true
);
...
function collisionEntries(state, excludeId = null) {
  ...
  for (const obstacle of STATIC_OBSTACLES) {
    if (obstacle.facility && !campaignFacilityVisible(state, obstacle.facility)) continue;
    entries.push({
...
function navigationRects(state, override = null) {
  ...
  for (const object of placedObjects(state)) {
    ...
    if (object.fixture && !fixtureIsInstalled(state, object.id)) continue;
    if (!object.collision?.blocksCustomers || object.render?.kind === 'existing'
      || object.id === override?.id) continue;
    if (object.fixture) result.push(...fixtureCollisionRects(object));
    else for (const volume of volumesFor(object, object.transform)) result.push(obbAabb(volume));
  }
  for (const extra of ensureLayout(state).extra) {
    if (extra.id !== override?.id) result.push(fixtureRect(extra));
  }
  // Core furniture is renderer-owned and therefore skipped above, but the
  // authored L-shaped front desk is still real navigation geometry. Keep both
  // analytic hulls in the same route proof used by build mode and customers.
  if (override?.id !== 'core-checkout-counter' || override?.place) {
    result.push({ ...FRONT_DESK_COLLIDERS.frontRun });
    result.push({ ...FRONT_DESK_COLLIDERS.returnRun });
  }
  if (override?.place) {
    const meta = objectMeta(state, override.id);
    if (meta?.collision?.blocksCustomers !== false) {
      const transformNow = override.place.transform || override.place;
      if (meta.fixture) {
        result.push(...fixtureCollisionRects({
          ...meta.fixture,
          x: transformNow.x,
          z: transformNow.z,
          ry: transformNow.ry || 0,
        }));
      } else {
        for (const volume of volumesFor(meta, transformNow)) result.push(obbAabb(volume));
      }
    }
  }
  return result;
...
export function routesIntact(state, override = null) {
  ...
  const fixtures = placedFixtures(state)
    .filter((fixture) => !override || fixture.id !== override.id);
  if (override?.place) {
    const meta = objectMeta(state, override.id);
    const transformNow = override.place.transform || override.place;
    fixtures.push(meta?.fixture
      ? { ...meta.fixture, ...transformNow }
      : transformNow);
  }
  ...
  for (let index = 0; index < 6; index += 1) {
    if (!reached(queueSlot(index))) return false;
  }
  if (!reached(COUNTER.staffStand)) return false;
  // This datum is the physical chair centre. The route only needs a legal
  // standing cell beside the seat, not inside its analytic footprint.
  if (!reached(FRONT_DESK.staffChair, 4)) return false;
...
function validateHorizontalSurface(state, meta, candidate, result) {
  ...
  const authoredCheckoutHardware = !!meta.requiredObject
...
export function validateObjectPlacement(state, id, candidateValue, options = {}) {
  ...
  // Runtime-owned reception components are aliases for the already-rendered
  // counter/chair/accessory roots. Their protected socket is validated above;
  // testing that alias against its own authoritative collider would make the
  // only legal pose fail as a self-overlap.
  if (meta.requiredObject !== 'front-desk-runtime') {
