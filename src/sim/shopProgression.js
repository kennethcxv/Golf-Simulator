...
import { FIXTURES, FURNISHED_CLUBHOUSE_FIXTURE_IDS } from '../data/shopLayout.js';
...
export function fixtureIsInstalled(state, fixtureId, tierId = null) {
  ...
  if (!tierId
      && state?.campaign?.furnishedStartVersion >= 1
      && FURNISHED_CLUBHOUSE_FIXTURE_IDS.includes(fixtureId)) return true;
...
export function shopCategoryUnlocked(state, category) {
  ...
  if (state?.campaign?.furnishedStartVersion >= 1
      && ['clubs', 'provisions'].includes(category)) return true;
