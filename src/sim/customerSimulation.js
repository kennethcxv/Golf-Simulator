...
export function scheduleReservationCustomer(state, reservation, options = {}) {
  ...
  const late = rng.chance(options.lateChance ?? 0.12);
  const offset = late ? rng.range(2, 12) : -rng.range(10, 20);
  const dayStart = reservation.dayAbs * 1440;
  pushArrival(sim, {
    dayAbs: reservation.dayAbs,
    scheduledMinute: dayStart + reservation.minute + offset,
    intendedMinute: dayStart + reservation.minute,
    intent: CUSTOMER_INTENT.RESERVATION_CHECK_IN,
    reservationId: reservation.id,
    name: reservation.name,
    partyId: `reservation-${reservation.id}`,
    partySize: Math.max(1, Math.min(4, reservation.partySize || 1)),
    noShow: rng.chance(options.noShowChance ?? 0.07),
...
export function cancelReservationCustomer(state, reservationId) {
  ...
  for (const arrival of sim.scheduled) {
    if (arrival.reservationId === reservationId && arrival.status === CUSTOMER_STATE.SCHEDULED) {
