...
const STATE_SPECS = {
  ...
  CardAmountEntry: {
    phase: 'card',
    branch: 'card',
    entryAction: 'Keep the card visibly inserted, prefill the exact cent total, and enable one confirmation action.',
    allowedInput: ['card-keypad-confirm', 'card-keypad-correction', 'cancel-card-at-reader'],
    camera: camera('card-reader-focus', 'hold', 'reader-constrained-look'),
    playerAnimation: 'card-inserted-keypad-entry',
    customerAnimation: 'watch-card-reader',
    uiState: ui('card-amount-entry', 'Total ready. Press the green key to confirm.'),
    audio: ['terminal-keypad'],
    completionCondition: 'The prefilled cent amount exactly matches the POS total and the player confirms it once.',
    timeout: noTimeout('The exact prefilled total waits safely for one deliberate confirmation.'),
