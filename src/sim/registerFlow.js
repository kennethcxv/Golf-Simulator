...
const STATE_SPECS = {
  ...
  WaitingForScan: {
    phase: 'scanning',
    branch: 'shared',
    entryAction: 'Enable only unscanned order products as draggable targets and highlight the current reachable product.',
    allowedInput: ['grab-product', 'look', 'exit-cashier'],
    ...
    uiState: ui('items-remaining', 'Pick up an item, turn its barcode toward the reader, scan it, then stage it.'),
    audio: ['scan-ready'],
    completionCondition: 'Player grabs one unscanned product belonging to this transaction exactly once.',
  ...
  ProductHeld: {
    phase: 'scanning',
    branch: 'shared',
    entryAction: 'Lock the grabbed product UID to the pointer while preserving its last safe counter pose.',
    allowedInput: ['move-held-product', 'rotate-held-product', 'release-held-product', 'cancel-held-product'],
    ...
    completionCondition: 'The pointer owns the correct product mesh and its authored barcode remains observable.',
  ...
  ProductScanning: {
    phase: 'scanning',
    branch: 'shared',
    entryAction: 'Track the held barcode through the physical scanner volume while the player continues the drag.',
    allowedInput: ['move-held-product', 'rotate-held-product', 'release-held-product', 'cancel-held-product'],
    camera: camera('scanner-focus', 'ease', 'limited-counter-look'),
    playerAnimation: 'hold-product-through-scanner',
    customerAnimation: 'look-at-scanner',
    uiState: ui('items-remaining', 'Barcode read. Stage the product on the packing strip.'),
    audio: ['scanner-activation'],
    completionCondition: 'The fresh owned UID crosses the scanner with a readable orientation and commits exactly once.',
  ...
  ProductScanned: {
    phase: 'scanning',
    branch: 'shared',
    entryAction: 'Commit one scan flag and append one POS line while the player retains the product for staging.',
    allowedInput: ['move-held-product', 'release-held-product', 'cancel-held-product'],
    camera: camera('cashier-wide', 'ease-out', 'limited-counter-look'),
    playerAnimation: 'stage-scanned-product',
    customerAnimation: 'look-at-bagging-area',
    uiState: ui('item-added', 'Item added to the order.'),
    audio: ['scanner-beep', 'pos-item-added'],
    completionCondition: 'The scanned product is visibly released in the staging strip and no longer intercepts unscanned product grabs.',
  ...
  AllProductsScanned: {
    phase: 'scanning',
    branch: 'shared',
    entryAction: 'Disable further scans and expose the deliberate total control after a short readable hold.',
    allowedInput: ['confirm-total', 'exit-cashier'],
    camera: camera('cashier-pos', 'ease', 'limited-counter-look'),
    playerAnimation: 'cashier-idle',
    customerAnimation: 'prepare-payment',
    uiState: ui('ready-for-payment', 'All items scanned. Press T to total the sale.'),
    audio: ['all-items-scanned'],
    completionCondition: 'Every transaction item is scanned and staged, then the player confirms the total.',
  ...
  CardPresented: {
    ...
    audio: ['card-present'],
    completionCondition: 'The card reaches the customer handoff pose and the physical terminal can be activated.',
    timeout: timed(30, 'The presented card was not taken.'),
    recoveryPath: recovery('CardPresented', 'card-unapproved', 'Return the card to its presentation pose and clear transient swipe progress.'),
  },
  CardSwipeReady: {
    phase: 'card',
    branch: 'card',
    entryAction: 'Keep the card in the customer handoff pose until the player activates the physical terminal, then place it at the top of the swipe channel.',
    allowedInput: ['activate-card-terminal', 'start-card-swipe', 'cancel-card-at-reader'],
    camera: camera('card-reader-focus', 'ease-in', 'reader-constrained-look'),
    playerAnimation: 'card-handoff-then-ready-in-channel',
    customerAnimation: 'watch-card-reader',
    uiState: ui('card-selected', 'Tap the terminal, then start at the top and swipe downward.'),
    audio: ['reader-ready'],
    completionCondition: 'The terminal is armed and the primary pointer catches the card at the top of the physical swipe channel.',
    timeout: noTimeout('The reader waits safely for a deliberate swipe.'),
    recoveryPath: recovery('CardSwipeReady', 'card-unapproved', 'Release pointer capture and return the card to the top of the swipe channel.'),
  },
  CardSwiping: {
    phase: 'card',
    branch: 'card',
    entryAction: 'Bind the card to the pointer and record its real top-to-bottom path through the reader channel.',
    allowedInput: ['move-card-swipe', 'release-card-swipe', 'cancel-card-at-reader'],
    camera: camera('card-reader-focus', 'hold', 'reader-constrained-look'),
    playerAnimation: 'swipe-card',
    customerAnimation: 'watch-card-reader',
    uiState: ui('card-selected', 'Swipe downward at a steady pace.'),
    audio: ['card-movement'],
    completionCondition: 'The released gesture reaches the bottom with valid direction, pace, and reversal tolerance.' ,
    timeout: timed(6, 'The held card stopped progressing through the reader.'),
    recoveryPath: recovery('CardSwipeReady', 'card-unapproved', 'Release the card and reset it to the top without authorizing payment.'),
  },
  CardProcessing: {
    phase: 'card',
    branch: 'card',
    entryAction: 'Freeze further swipe input, light the terminal processing state, and start one authorization request.',
    allowedInput: ['exit-cashier'],
    camera: camera('card-reader-focus', 'hold', 'reader-constrained-look'),
    playerAnimation: 'card-swiped-no-hands',
  ...
  CashAccepted: {
    phase: 'cash',
    branch: 'cash',
    entryAction: 'Checkpoint tender ownership and lay every received piece within reach on the counter.',
    allowedInput: ['open-cash-drawer', 'exit-cashier'],
    camera: camera('drawer-preparation', 'ease', 'limited-drawer-look'),
    playerAnimation: 'hold-cash',
    customerAnimation: 'wait-for-change',
    uiState: ui('cash-received', 'Cash received. Press D to open the drawer.'),
    audio: ['cash-accepted'],
    completionCondition: 'The player deliberately opens the drawer while every accepted tender piece remains accounted for.',
  ...
  DepositingCash: {
    phase: 'cash',
    branch: 'cash',
    entryAction: 'Enable each received piece as a draggable target and every matching denomination well as a drop target.',
    allowedInput: ['grab-tender-piece', 'move-tender-piece', 'release-tender-piece', 'exit-cashier'],
    camera: camera('open-drawer-focus', 'hold', 'drawer-constrained-look'),
    playerAnimation: 'drag-cash-to-matching-well',
    customerAnimation: 'wait-for-change',
    uiState: ui('cash-received', 'Deposit each received piece into its matching compartment.'),
    audio: ['paper-bill', 'coin-drop'],
    completionCondition: 'Every tender piece is deliberately released over its matching transaction-local drawer compartment.',
  ...
  SelectingChange: {
    ...
    allowedInput: ['select-change-piece', 'undo-change-piece', 'clear-change', 'click-customer-palm'],
    camera: camera('change-selection-focus', 'ease', 'drawer-constrained-look'),
    playerAnimation: 'select-and-hold-change',
    customerAnimation: 'wait-for-change',
    uiState: ui('change-required', 'Select the required change, then click the customer\'s open palm.'),
    audio: ['bill-pickup', 'coin-pickup', 'change-total-update'],
    completionCondition: 'The selected change is within policy and the player hands it to the customer\'s open palm; exact cash still requires that deliberate acknowledgement.',
  ...
  GivingChange: {
    phase: 'cash',
    branch: 'cash',
    entryAction: 'Animate the player-confirmed change bundle from the cashier hand target to the customer reception target.',
    allowedInput: [],
  ...
  ReceiptPrinting: {
    phase: 'receipt',
    branch: 'shared',
    entryAction: 'Animate exactly one receipt progressively through the printer, then expose it as a physical pickup target.',
    allowedInput: ['grab-printed-receipt', 'move-receipt', 'release-receipt'],
    ...
    completionCondition: 'The receipt fully emerges once and the player takes it without creating a duplicate.',
  ...
  Bagging: {
    phase: 'bagging',
    branch: 'shared',
    entryAction: 'Expose the printed receipt and every paid staged product as physical targets around the open bag.',
    allowedInput: ['drag-receipt-to-bag', 'drag-product-to-bag', 'grab-filled-bag-handles'],
    camera: camera('bagging-focus', 'ease-in', 'limited-bag-look'),
    playerAnimation: 'pack-open-bag',
    customerAnimation: 'wait-for-bag',
    uiState: ui('bagging', 'Pack the receipt and every product into the bag.'),
    audio: ['bag-open', 'bag-rustle', 'item-bagged'],
    completionCondition: 'The player places every compact product and the receipt in the bag, with oversize products in a separate carry state.',
  ...
  BagHandoff: {
    phase: 'bagging',
    branch: 'shared',
    entryAction: 'Bind the completed bag handles to the pointer for an offer to the customer\'s authored carry grip.',
    allowedInput: ['move-filled-bag', 'release-filled-bag', 'cancel-filled-bag'],
...
const FORWARD_TRANSITIONS = {
  ...
  CardPresented: ['CardSwipeReady', 'ChoosingPayment', 'AllProductsScanned'],
  CardSwipeReady: ['CardSwiping', 'ChoosingPayment', 'AllProductsScanned'],
  CardSwiping: ['CardSwipeReady', 'CardProcessing', 'AllProductsScanned'],
...
export function validateCheckoutContract() {
  const errors = [];
  if (CHECKOUT_STATE_ORDER.length !== 29) errors.push(`Expected 29 checkout states, found ${CHECKOUT_STATE_ORDER.length}.`);
