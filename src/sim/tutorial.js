...
export const CONTEXTUAL_TUTORIALS = [
  ...
  {
    id: 'refinish-paint', context: 'walk', title: 'Refinish repaired surfaces',
    hint: 'A repaired component can be refinished: press E at it to apply the next finish. Each application is paid from club funds.',
    completeFlag: 'paintedComponent',
  },
];
...
export function tutorialFlag(state, flag) {
  ...
  // The campaign's arrival survey listens for the same look-around behavior
  // the tutorial detects; this forwarding is the reason recordCampaignEvent
  // is imported here. recordCampaignEvent itself refuses inactive campaigns.
  if (flag === 'lookedAround') recordCampaignEvent(state, 'lookedAround');
