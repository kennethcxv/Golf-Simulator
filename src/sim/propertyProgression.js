...
export function ensureEmpireProgression(empire) {
  if (!empire.progression || typeof empire.progression !== 'object' || Array.isArray(empire.progression)) {
    initEmpireProgression(empire);
  }
  const progression = empire.progression;
  if (!Array.isArray(progression.unlockedTierIds)) progression.unlockedTierIds = ['neglectedPublic'];
  if (!progression.unlockedTierIds.includes('neglectedPublic')) progression.unlockedTierIds.unshift('neglectedPublic');
  if (!Array.isArray(progression.completedSales)) progression.completedSales = [];
  if (!progression.processedSaleIds || typeof progression.processedSaleIds !== 'object'
      || Array.isArray(progression.processedSaleIds)) progression.processedSaleIds = {};
  if (!Array.isArray(progression.appraisals)) progression.appraisals = [];
  ...
  if (!Array.isArray(progression.saleBackups)) progression.saleBackups = [];
  return progression;
}
