...
export function ensureCleaningToolState(state) {
  ...
  // The last usable wring can leave 0.01 capacity after rounded 0.055 steps.
  // That remainder is below the service threshold, so persist one canonical
  // empty state instead of a dirty-looking bucket that can never be used.
  if (c.bucket.level <= 0.02) c.bucket.level = 0;
