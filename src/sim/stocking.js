...
// Shared physical transfer used by both authored fixtures and player-placed
// retail shelves. The caller owns compatibility and capacity; this seam owns
// carry conservation plus the lot-ledger movement.
export function moveCarriedToShelf(state, units = 1, reason = 'Stocked a retail display') {
  const c = carriedGoods(state);
  if (!c) return { ok: false, reason: 'Your hands are empty.' };
  const inv = state.shop?.inventory?.[c.skuId];
  if (!inv) return { ok: false, reason: 'That carried product has no inventory record.' };
  const moved = Math.min(Math.max(0, Math.floor(units)), c.qty);
  if (moved < 1) return { ok: false, reason: 'There is no room for another product.' };
  ...
  return {
    ...
    allocations: ledgerMove.allocations,
  };
}
...
export function stockFixture(state, fixtureId, units = 1) {
  ...
  const restoration = syncStarterRestockMilestones(state, c.skuId);
  return {
    ok: true,
...
export function takeFromBack(state, skuId, want = 999) {
  ...
  const allocation = allocationsForStage(state, {
    ...
    quantity: taken,
    skuId,
    excludeAllocations: c?.skuId === skuId ? c.allocations : null,
  });
