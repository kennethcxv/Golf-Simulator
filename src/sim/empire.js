...
const LEGACY_STARTING_PROPERTY_NAMES = new Set([
...
export function initPropertyState(property, mode) {
  ...
  state.property.tierId = property.tierId || state.property.tierId || null;
...
// Player-facing New Game starts at the authored fixer-upper immediately. Keep
// newEmpire() as the empty-portfolio primitive for save migration and economy
// tests; this helper is the deliberate onboarding transaction used by main.js.
export function newStarterEmpire(mode = 'relaxed', seed = Date.now() % 2147483647) {
  const empire = newEmpire(mode, seed);
  const purchase = buyProperty(empire, STARTING_PROPERTY_ID);
  if (!purchase.ok) {
    throw new Error(`Could not start at ${STARTING_PROPERTY_NAME}: ${purchase.reason}`);
  }
  return empire;
}
...
function legacyEmpireFromState(st) {
  ...
  const record = {
    ...
    size: st.course.holes.length >= 14 ? 18 : st.course.holes.length === 3 ? 3 : 9,
    seed: st.seed,
    layout: { kind: 'willow' },
...
function deserializeEmpireLegacyUnsafe(raw) {
...
function normalizedMarketProperty(value, index, seed, listedDay, report) {
  ...
  assign('name', property.id === STARTING_PROPERTY_ID
    ? STARTING_PROPERTY_NAME
  ...
  assign('size', property.size === 18
    ? 18
    : property.id === STARTING_PROPERTY_ID && property.size === 3 ? 3 : 9);
...
function normalizedProperty(value, state, index, seed, report) {
  ...
  const persistedName = typeof property.name === 'string' && property.name.trim()
    ? property.name.slice(0, 200)
    : state.clubName;
  assign('name', property.id === STARTING_PROPERTY_ID
      && LEGACY_STARTING_PROPERTY_NAMES.has(String(persistedName).trim())
    ? STARTING_PROPERTY_NAME
    : persistedName);
  assign('blurb', typeof property.blurb === 'string' ? property.blurb.slice(0, 2000) : '');
  assign('size', property.size === 18
    ? 18
    : property.id === STARTING_PROPERTY_ID
        && (property.size === 3 || state.course?.holes?.length === 3) ? 3 : 9);
  assign('seed', finiteNumber(property.seed, seed + index + 1, {
    integer: true, min: 1, max: 2147483647,
  }));
  property.layout = normalizedPropertyLayout(
...
const EMPIRE_SAVE_KEYS = new Set([
  ...
  'firstPurchaseDone', 'log', 'market', 'marketRngState', 'lastMarketDay',
  'marketCondition', 'marketConditionTarget', 'progression', 'holdings',
]);
const HOLDING_SAVE_KEYS = new Set(['property', 'passive', 'state']);
const UNSAFE_SAVE_KEYS = new Set(['__proto__', 'prototype', 'constructor']);

function preserveUnknownSaveFields(target, source, knownKeys) {
...
export function deserializeEmpireWithReport(raw) {
  ...
  const empire = {
    ...
    progression: isRecord(data.progression) ? cloneSaveValue(data.progression, {}) : null,
  };
  ensureEmpireProgression(empire);
  preserveUnknownSaveFields(empire, data, EMPIRE_SAVE_KEYS);
