  ...
  if (hole.green != null) {
    if (!isRecord(hole.green)) {
      ...
      if (!pts) {
        ...
        if (!pinsWereArray) noteRepair(report, `${path}.green.pins`, 'invalid pin array defaulted');
      }
    }
  }
...
function normalizeHole(hole, report, path) {
  ...
  for (const [containerKey, names] of [['tees', ['back', 'middle', 'forward']], ['pins', ['A', 'B', 'C']]]) {
    ...
    for (const name of names) {
      ...
      normalized[name] = normalizePoint(source, report, `${path}.${containerKey}.${name}`);
...
function normalizeCourseVector(value, seed, report) {
      ...
      accept: (feature) => {
        ...
        if (key === 'waters') {
          ...
          const kind = typeof feature.kind === 'string' && feature.kind.trim()
            ...
            : 'pond';
          if (kind !== feature.kind) noteRepair(report, '$.course.vec.waters[].kind', 'invalid water kind normalized');
          feature.kind = kind;
        } else if (key === 'streams') {
  ...
  vector.mounds = normalizeIds(vector.mounds, report, '$.course.vec.mounds', {
    ...
    accept: (mound) => {
      ...
      mound.x = finiteSave(mound.x, 0, { min: -10_000, max: 10_000 }, report, '$.course.vec.mounds[].x');
      mound.y = finiteSave(mound.y, 0, { min: -10_000, max: 10_000 }, report, '$.course.vec.mounds[].y');
      mound.r = finiteSave(mound.r, 1, { min: 0.01, max: 1000 }, report, '$.course.vec.mounds[].r');
      mound.h = finiteSave(mound.h, 0, { min: -1000, max: 1000 }, report, '$.course.vec.mounds[].h');
      return true;
    },
  });
  ...
  for (const lawn of rawLawns) {
    ...
    vector.lawns.push({
      ...lawn,
      x: finiteSave(lawn.x, 0, { min: -10_000, max: 10_000 }, report, '$.course.vec.lawns[].x'),
      y: finiteSave(lawn.y, 0, { min: -10_000, max: 10_000 }, report, '$.course.vec.lawns[].y'),
      w: finiteSave(lawn.w, 1, { min: 0.01, max: 1000 }, report, '$.course.vec.lawns[].w'),
      d: finiteSave(lawn.d, 1, { min: 0.01, max: 1000 }, report, '$.course.vec.lawns[].d'),
      rot: finiteSave(lawn.rot, 0, { min: -Math.PI * 8, max: Math.PI * 8 }, report, '$.course.vec.lawns[].rot'),
    });
  }
...
function normalizeCourse(savedCourse, seed, persistedVersion, report) {
  ...
  for (let index = 0; index < rawStructures.length; index += 1) {
    ...
    structures.push({
      ...
      x: finiteSave(structure.x, 0, { min: -10_000, max: 10_000 }, report, `$.course.structures[${index}].x`),
      y: finiteSave(structure.y, 0, { min: -10_000, max: 10_000 }, report, `$.course.structures[${index}].y`),
      w: finiteSave(structure.w, 1, { min: 0.01, max: 10_000 }, report, `$.course.structures[${index}].w`),
      h: finiteSave(structure.h, 1, { min: 0.01, max: 10_000 }, report, `$.course.structures[${index}].h`),
    });
  }
  ...
  const objects = hadPersistedObjects
    ? normalizeIds(savedCourse.objects, report, '$.course.objects', {
      ...
      accept: (object) => {
        ...
        object.x = finiteSave(object.x, 0, { min: -10_000, max: 10_000 }, report, '$.course.objects[].x');
        object.y = finiteSave(object.y, 0, { min: -10_000, max: 10_000 }, report, '$.course.objects[].y');
        object.rot = finiteSave(object.rot, 0, { min: -Math.PI * 100, max: Math.PI * 100 }, report, '$.course.objects[].rot');
        object.scale = finiteSave(object.scale, 1, { min: 0.01, max: 100 }, report, '$.course.objects[].scale');
        if (object.type !== originalType) noteRepair(report, '$.course.objects[].type', 'invalid object type normalized');
        return true;
      },
    })
    : [];
  const paths = normalizeIds(savedCourse.paths, report, '$.course.paths', {
    ...
    accept: (path) => {
      ...
      if (!pts) return false;
      path.pts = pts;
      path.width = finiteSave(path.width, 3.2, { min: 0.1, max: 500 }, report, '$.course.paths[].width');
      const material = typeof path.material === 'string' && path.material.trim()
        ? path.material.slice(0, 100)
        : 'asphalt';
      if (material !== path.material) noteRepair(report, '$.course.paths[].material', 'invalid path material normalized');
...
function normalizeTurf(rawTurf, defaults, report) {
  ...
  return {
    ...
    // A save without explicit surface-damage fields predates that system. It
    // must gain empty ledgers, not the seeded wear from the temporary new-game
    // instance used to supply unrelated turf defaults.
    divots: normalizeNumericArray(rawTurf.divots, new Float32Array(defaults.health.length), Float32Array, report, '$.turf.divots', pct),
    ballMarks: normalizeNumericArray(rawTurf.ballMarks, new Float32Array(defaults.health.length), Float32Array, report, '$.turf.ballMarks', pct),
  };
}
...
export function deserializeWithReport(json) {
  ...
  // These domains use absence itself as their migration signal. Do not let
  // newGame's modern defaults disguise a genuinely older save.
