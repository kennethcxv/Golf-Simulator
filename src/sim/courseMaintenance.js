...
export function snapshotCourseMaintenance(model) {
  ...
  // Some callers deliberately snapshot an already serialized GameState while
  // testing persistence idempotence. Its maintenance fields are encoded
  // strings, not live typed arrays; preserve that valid representation rather
  // than trying to encode it a second time.
  if (model.fields && Object.keys(SERIAL_FIELDS).some((name) => !model[name])) {
