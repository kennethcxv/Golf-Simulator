...
export function consumeHeldBatch(state, items) {
  ...
  const sold = moveInventory(state, {
    ...
    allocations,
    referenceId: `checkout-sale-batch:${selected.map((entry) => entry.uid).sort().join('|')}`,
...
export function checkoutSale(state, items, who = 'A customer', transactionId = null, options = {}) {
  ...
  const allocationBatch = allocationBatchForHeld(state, heldMatches);
  if (!allocationBatch.ok) return allocationBatch;
  const { allocations } = allocationBatch;
  let itemTotal = 0;
  ...
  const generatedTransactionId = id == null;
  const nextTransactionId = Number.isInteger(state.shop.nextTransactionId)
    && state.shop.nextTransactionId > 0
    ? state.shop.nextTransactionId
    : 1;
  const propertyId = state.property?.id || `club-${state.seed}`;
  if (id == null) id = `${propertyId}:legacy-register-${nextTransactionId}`;
  ...
  if (!revenuePreflight.ok) return revenuePreflight;
  if (revenuePreflight.duplicate) {
    return { ok: false, reason: 'Already banked.', duplicate: true, transactionId: id };
  }
