...
// Pine Hills is sold as a neglected municipal fixer-upper. Turf condition and
// disease already carry the agronomy state, but without any bare ground the
// authored course still reads freshly renovated from every player camera.
// These restrained, route-relative-looking scars sit in ordinary turf only;
// analytic tees, greens, hazards, and cart paths retain priority in
// evaluateSurface(). Keeping the sockets fixed also makes their save/editor
// paint authority deterministic without consuming the course RNG stream.
const STARTER_NEGLECT_WEAR = Object.freeze([
  Object.freeze({ x: 31.4, y: 32.7, rx: 1.55, ry: 0.82, angle: -0.18 }),
  Object.freeze({ x: 47.8, y: 31.2, rx: 1.85, ry: 0.92, angle: 0.08 }),
  Object.freeze({ x: 62.0, y: 29.7, rx: 1.35, ry: 0.72, angle: -0.14 }),
  Object.freeze({ x: 84.6, y: 32.0, rx: 1.42, ry: 0.82, angle: -0.22 }),
  Object.freeze({ x: 91.0, y: 30.4, rx: 1.18, ry: 0.68, angle: -0.2 }),
  Object.freeze({ x: 95.0, y: 18.0, rx: 1.55, ry: 0.82, angle: 0.38 }),
  Object.freeze({ x: 78.5, y: 13.2, rx: 1.9, ry: 0.96, angle: 0.08 }),
  Object.freeze({ x: 58.0, y: 12.2, rx: 1.48, ry: 0.76, angle: -0.04 }),
]);

function paintStarterNeglectWear(course) {
  const paint = ensurePaint(course);
  const paintable = new Set([ZONE.FAIRWAY, ZONE.SEMI, ZONE.ROUGH, ZONE.HEAVY]);
  for (let patchIndex = 0; patchIndex < STARTER_NEGLECT_WEAR.length; patchIndex++) {
    const patch = STARTER_NEGLECT_WEAR[patchIndex];
    const ca = Math.cos(patch.angle);
    const sa = Math.sin(patch.angle);
    const radius = Math.max(patch.rx, patch.ry) + 1;
    const x0 = Math.max(0, Math.floor(patch.x - radius));
    const y0 = Math.max(0, Math.floor(patch.y - radius));
    const x1 = Math.min(course.w - 1, Math.ceil(patch.x + radius));
    const y1 = Math.min(course.h - 1, Math.ceil(patch.y + radius));
    for (let y = y0; y <= y1; y++) {
      for (let x = x0; x <= x1; x++) {
        const dx = x + 0.5 - patch.x;
        const dy = y + 0.5 - patch.y;
        const across = (dx * ca + dy * sa) / patch.rx;
        const along = (-dx * sa + dy * ca) / patch.ry;
        // A fixed hash breaks up the ellipse enough to read as worn turf while
        // preserving broad, smooth boundaries in the high-resolution field.
        const hash = Math.sin(
          (x + 1) * 12.9898 + (y + 1) * 78.233 + patchIndex * 19.19,
        ) * 43758.5453;
        const edgeJitter = (hash - Math.floor(hash) - 0.5) * 0.22;
        if (across * across + along * along + edgeJitter > 1) continue;
        const index = y * course.w + x;
        if (paintable.has(course.zones[index])) paint[index] = ZONE.DIRT;
      }
    }
  }
  deriveZones(course);
...
export function designCourse(rng, opts = {}) {
  ...
  const holeCount = Number.isFinite(opts.holeCount)
    ? Math.max(1, Math.min(TEMPLATE.length, Math.floor(opts.holeCount)))
    : TEMPLATE.length;
  const template = holeCount === TEMPLATE.length ? TEMPLATE : TEMPLATE.slice(0, holeCount);
  ...
  let h6PathStart = -1;
  let h8Exit = -1;
  // H1-H8 form the full nine's main circulation graph. H9 is appended later as a branch
  // from the H5/H6 spine, preventing a sequential spline from lapping the east
  // boundary and crossing the H6 target view. Compact starter layouts use the
  // same authored controls in sequence and therefore include every designed hole.
  const fullNineRouting = designed.length === TEMPLATE.length;
  const mainPathHoleCount = fullNineRouting ? designed.length - 1 : designed.length;
  for (let i = 0; i < mainPathHoleCount; i++) {
    const d = designed[i];
  ...
  deriveZones(course);
  if (opts.neglectedWear) paintStarterNeglectWear(course);
