...
// Lifecycle events are diagnostics, not the conservation authority. Retain a
// useful recent window without letting routine carton animation dominate saves.
...
// Once a physical carton has reached its terminal recycling sink, none of its
// receive/open/take commands can be reached again: the durable arrival ID and
// monotonically increasing box ID remain the replay guards. Retire only those
// box-scoped results so mid-unboxing saves keep full idempotency while completed
// cartons do not leave an ever-growing command journal.
export function forgetInventoryOperationsForBox(state, boxId) {
  const lifecycle = ensureInventoryLifecycle(state);
  if (!lifecycle || !Number.isSafeInteger(boxId)) return 0;
  const exact = new Set([`receive-box:${boxId}`, `open-box:${boxId}`]);
  const prefixes = [`take-box:${boxId}:`, `employee-unpack-box:${boxId}:`];
  const retired = lifecycle.operationKeys.filter((key) => (
    exact.has(key) || prefixes.some((prefix) => key.startsWith(prefix))
  ));
  if (!retired.length) return 0;
  const retiredSet = new Set(retired);
  for (const key of retired) delete lifecycle.operations[key];
  lifecycle.operationKeys = lifecycle.operationKeys.filter((key) => !retiredSet.has(key));
...
export function ensureArrivalOrder(state, input) {
  ...
  order.externalCompatibility = true;
...
export function refreshOrderFromLedger(state, orderOrId) {
  ...
  if (inTransit > 0 && inTransit < total) {
    ...
    if (unpacked >= total) {
      ...
      if (order.externalCompatibility === true
          || (order.charged === false && lots.every((lot) => lot.source === 'external-order'))) {
        const compact = {
          id: order.id,
          externalCompatibility: true,
          skuId: order.skuId || (order.lines?.length === 1 ? order.lines[0].skuId : null),
          qty: total,
          quantity: total,
          lines: (order.lines || []).map((line) => ({
            ...
            unpackedQuantity: line.quantity,
          })),
          receivedManifestBoxIndexes: Array.isArray(order.receivedManifestBoxIndexes)
            ? [...order.receivedManifestBoxIndexes]
            : [],
          state: ORDER_STATE.FULLY_UNPACKED,
          completionState: ORDER_STATE.FULLY_UNPACKED,
          deliveredState: ORDER_STATE.DELIVERED,
          receivingState: ORDER_STATE.RECEIVED,
          remainingUnreceivedQuantity: 0,
          receivedQuantity: total,
          unpackedQuantity: total,
          charged: false,
          refunded: false,
        };
        for (const key of Object.keys(order)) delete order[key];
        Object.assign(order, compact);
      }
    } else if (unpacked > 0) {
      order.receivingState = ORDER_STATE.RECEIVED;
