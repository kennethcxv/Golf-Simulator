...
function placedFixturesReadOnly(state) {
  ...
  for (const base of FIXTURES) {
    if (!fixtureIsInstalled(state, base.id)) continue;
...
function resolveSurfaceDescriptor(state, template, options = {}) {
  ...
  return {
    ...
    worldPose: composePose(parent.pose, template.localPose),
  };
}
...
function fixedFloorBlockers(state, options) {
  const blockers = [
    ...
    )] : []),
    {
      ...
      rect: {
        minX: FRONT_DESK_COLLIDERS.returnRun.minX - 0.15,
        maxX: FRONT_DESK_COLLIDERS.returnRun.maxX + 0.15,
        minZ: FRONT_DESK_COLLIDERS.returnRun.minZ - 0.10,
        maxZ: FRONT_DESK_COLLIDERS.returnRun.maxZ + 0.10,
      },
    },
    fixedBlocker(
  ...
  if (campaignFacilityVisible(state, 'officeDesk')) {
    blockers.push(fixedBlocker(
  ...
  if (!['resortStyle', 'premiumPrivate'].includes(state.property?.tierId)) {
    ...
    blockers.push(
      fixedBlocker(
        'restroom-east-wall', 'restroom wall',
        restroom.eastWallX,
      ...
      fixedBlocker(
        'restroom-south-wall', 'restroom wall',
      ...
      fixedBlocker(
        'restroom-north-wall', 'restroom wall',
        restroom.crossWallCenterX,
        restroom.northWallZ,
        restroom.crossWallWidth,
        restroom.wallThickness,
      ...
      fixedBlocker(
        'restroom-toilet', 'restroom toilet',
        restroom.toilet.x, restroom.toilet.z, restroom.toilet.w, restroom.toilet.d,
      ),
      fixedBlocker(
        'restroom-sink', 'restroom hand basin',
        restroom.sink.x, restroom.sink.z, restroom.sink.w, restroom.sink.d,
      ),
    );
  }
  for (const [id, dimensions, fallback] of [
    [STOCKING_CART_EQUIPMENT_ID, [1.00, 0.50], { x: 6.35, y: 0, z: -3.4, ry: 0 }],
    ['delivery_hand_truck', [0.50, 0.45], {
...
function floorRouteIntact(state, candidateRect, selfId, options) {
  // Some migrated/authored layouts predate this conservative grid proof. If
  // the route is already invalid, keep enforcing overlap and support rules but
  // do not let that inherited mismatch brick every safe carton set-down.
  ...
  for (const [target, searchRadius] of [
    ...
    // The reception-chair coordinate is its physical centre. Reachability means
    // a person can stand beside it, not inside its authoritative footprint.
    [FRONT_DESK.staffChair, 4],
