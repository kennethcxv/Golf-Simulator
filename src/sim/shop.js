...
import {
  addCostOfGoods, addExpense, addRevenue, preflightLedgerEntry, recordOutcome, unbill,
} from './economy.js';
...
import { syncGenericCleanupMilestone, totalDebris } from './cleaningDebris.js';
...
import { syncStarterRestockMilestones } from './clubhouseStarterStock.js';
...
export const RENO = {
  clutterLayout: 4,           // v4 also clears the employee-room doorway
...
export function wipeWindow(state, index) {
  ...
  const restoration = left <= 0.01 && reno.windows.every((value) => value <= 0.01)
    ? restorationAction(state, { type: 'complete-cleanup-milestone', milestoneId: 'windows' })
    : null;
  return { ok: true, left, restoration };
}
...
const conditionRatio = (completed, total) => (
  total > 0 ? clamp(completed / total, 0, 1) : 1
);

export function shopConditionBreakdown(state) {
  const reno = state.shop && state.shop.reno;
  if (!reno) {
    return {
      floor: 0,
  ...
  const restoration = ensureClubhouseRestoration(state);
  const grime = Array.isArray(reno.grime) ? reno.grime : [];
  const avgDirt = grime.length
    ? grime.reduce((sum, value) => sum + clamp(Number(value) || 0, 0, 1), 0) / grime.length
    : 0;
  const windows = Array.isArray(reno.windows) ? reno.windows : [];
  const avgWindowFilm = windows.length
    ? windows.reduce((sum, value) => sum + clamp(Number(value) || 0, 0, 1), 0) / windows.length
    : 0;
  const clutter = Array.isArray(reno.clutter) ? reno.clutter : [];
  const clutterClean = conditionRatio(clutter.filter((entry) => entry?.cleared).length, clutter.length);
  const debrisClean = restoration?.cleanupMilestones?.['generic-debris'] === true
    || !Array.isArray(reno.debris)
    || totalDebris(state) <= 0.02
    ? 1
    : 0;
  const discreteProgress = restoration
    ? CLUBHOUSE_CLEANUP_TARGET_IDS.reduce(
      (sum, targetId) => sum + clamp(restoration.targetProgress[targetId] || 0, 0, 1),
      0,
    )
    : 0;
  const lightProgress = restoration
    ? CLUBHOUSE_LIGHT_TARGET_IDS.filter((targetId) => (
      restoration.targetProgress[targetId] >= 1
      && restoration.lightPanels[targetId.replace('ceiling:', '')] === 'working'
    )).length
    : 0;
  const restockProgress = restoration
    ? CLUBHOUSE_RESTOCK_GROUP_IDS.filter((groupId) => restoration.restockMilestones[groupId]).length
    : 0;
  let finish = 0;
  ...
  const breakdown = {
    floor: clamp(1 - avgDirt, 0, 1) * 35,
    windows: clamp(1 - avgWindowFilm, 0, 1) * 8,
    genericCleanup: ((clutterClean + debrisClean) / 2) * 8,
    discreteCleanup: conditionRatio(discreteProgress, CLUBHOUSE_CLEANUP_TARGET_IDS.length) * 8,
    lighting: conditionRatio(lightProgress, CLUBHOUSE_LIGHT_TARGET_IDS.length) * 5,
    restocking: conditionRatio(restockProgress, CLUBHOUSE_RESTOCK_GROUP_IDS.length) * 6,
    decoration: Math.min(finish, RENO.finishCap),
  };
  breakdown.cleanliness = breakdown.floor
    + breakdown.windows
    + breakdown.genericCleanup
    + breakdown.discreteCleanup
    + breakdown.lighting
    + breakdown.restocking;
  breakdown.total = breakdown.cleanliness + breakdown.decoration;
...
export function shopCondition(state) {
  return Math.round(shopConditionBreakdown(state).total);
}
...
export function cleanGrimeAt(state, x, z, amount) {
  ...
  const restoration = cleaned > 0 && reno.grime.every((value) => value <= 0.01)
    ? restorationAction(state, { type: 'complete-cleanup-milestone', milestoneId: 'floor' })
    : null;
  return { cleaned, restoration };
}
...
export function clearClutter(state, idx) {
  ...
  const restoration = syncGenericCleanupMilestone(state);
...
export function placeOrder(state, skuId, qty, { service = 'standard' } = {}) {
  ...
  const legacySupplierOverride = state.tutorial?.complete && state.shop.unlockedTier >= 3;
  if (RETAIL_CATS.has(sku.cat) && !shopCategoryUnlocked(state, sku.cat) && !legacySupplierOverride) {
  ...
  const lifecycleStarter = isStarterDelivery(state);
  const lifecycleEta = quoteDelivery(state, sku, qty, {
    service,
  ...
  const lifecycleSurcharge = chargeOrderSurcharge(
    state, purchasedOrder, lifecycleEta.expressFee, 'express',
  );
  if (!lifecycleSurcharge.ok) {
    cancelPurchaseOrder(state, purchasedOrder.id);
    return lifecycleSurcharge;
  }
  if (lifecycleStarter && state.shop.starterOrderMin == null) {
    state.shop.starterOrderMin = state.clock.minutes;
  }
  Object.assign(purchasedOrder, {
    service: lifecycleEta.service,
    pace: lifecycleEta.pace,
    expressFee: lifecycleEta.expressFee,
    arrivesDay: lifecycleEta.arrivesDay,
    placedMin: lifecycleEta.placedMin,
    deliveryEtaMin: lifecycleEta.deliveryMin,
    deliveryMin: lifecycleEta.deliveryMin,
    window: { ...lifecycleEta.window },
    timing: {
      processingMinutes: lifecycleEta.processingMinutes,
      transitMinutes: lifecycleEta.transitMinutes,
      dispatchMin: lifecycleEta.dispatchMin,
      supplierSpeedModifier: lifecycleEta.supplierSpeedModifier,
      expressModifier: lifecycleEta.expressModifier,
    },
    status: 'received',
    notif: {},
  });
  ...
  if (state.campaign?.enabled && !state.campaign.businessOpen) {
    ...
    const processingMinutes = Math.max(
      1, Math.floor((deliveryMin - purchasedOrder.placedMin) * 0.4),
    );
    ...
    if (sku.campaign) {
      ...
      state.campaign.purchased[skuId] = Math.max(
        0, Math.floor(Number(state.campaign.purchased[skuId]) || 0),
      ) + qty;
...
export function cancelOrder(state, id) {
  ...
  // the code below remains only for pre-lifecycle save records.
  if (Array.isArray(o.lines)) {
    const cancelled = cancelPurchaseOrder(state, id);
    ...
    if (o.campaignTrackedQty && state.campaign?.purchased) {
      state.campaign.purchased[o.skuId] = Math.max(
        0,
        Math.floor(Number(state.campaign.purchased[o.skuId]) || 0)
          - Math.floor(o.campaignTrackedQty),
      );
    }
    return cancelled;
  }
...
export function tickDeliveries(state, nowMin) {
  ...
  let reserved = 0;
  const existingPadBoxes = state.shop.deliveries?.boxes?.filter((box) => box.loc === 'pad') || [];
  // Legacy/unpositioned pad boxes do not prove that the overflow receiving
  // lane is accessible. Managed arrivals have stable pad slots and may use it.
  const managedReceiving = existingPadBoxes.every((box) => Number.isSafeInteger(box.receivingSlot));
  for (const o of state.shop.orders) {
    ...
    if (o.status === 'delivered') {
      ...
      const available = managedReceiving ? receivingFree(state) : PAD_CAPACITY - padCount(state);
...
export function deliverOrdersDue(state, dayAbs) {
  ...
  for (const order of state.shop.orders) {
    ...
    const need = Math.max(0, totalBoxes - receivedBoxes);
      const free = Math.max(0, PAD_CAPACITY - padCount(state) - reserved);
    if (free <= 0) {
...
export function restockShelfFromBackroom(state, skuId) {
  ...
  inv.back -= move;
  inv.shelf += move;
  return { ok: true, moved: move, restoration: syncStarterRestockMilestones(state, skuId) };
}
...
export function restockShelvesByStaff(state) {
  ...
  for (const sku of SHOP_CATALOG) {
    ...
    if (move > 0) {
      ...
      capacity -= move;
      moved += move;
      syncStarterRestockMilestones(state, sku.id);
    }
  }
  return moved;
}
...
export function shopDailyAccrual(state) {
  ...
  const salesPostingId = `shop-simulation:${dayAbs}:sales`;
  const priorSaleEntryId = state.ledger?.processedIds?.[salesPostingId];
  const priorSale = priorSaleEntryId
    ? state.ledger.entries?.find((entry) => entry.id === priorSaleEntryId) || null
    : null;
  if (priorSale) {
    const priorCogsEntryId = state.ledger.processedIds?.[`shop-simulation:${dayAbs}:cogs`];
    const priorCogs = state.ledger.entries?.find((entry) => entry.id === priorCogsEntryId) || null;
    shop.lastAccruedDay = dayAbs;
    shop.lastAccrual = {
      dayAbs,
      shoppers: Math.max(0, Math.floor(priorSale?.customerCount || priorSale?.units || 0)),
      units: Math.max(0, Math.floor(priorSale?.units || 0)),
      lost: 0,
      lostToStock: 0,
      lostToPrice: 0,
      revenue: Math.max(0, Number(priorSale?.amount) || 0),
      costOfGoods: Math.max(0, Number(priorCogs?.amount) || 0),
      rentalRevenue: 0,
      fittings: 0,
  ...
  let lostToStock = 0;
  let lostToPrice = 0;
  const virtualShelf = new Map(SHOP_CATALOG.map((sku) => [
    sku.id,
    Math.max(0, Math.floor(Number(shop.inventory[sku.id]?.shelf) || 0)),
  ]));
  const salesPlan = [];
  shop.log = [];
  ...
  for (let i = 0; i < shoppers; i++) {
    ...
    const options = (catalogByCat[cat] || []).filter((s) => {
      ...
      return skuDisplayIsInSet(s.id, floorFixtures) && (virtualShelf.get(s.id) || 0) > 0;
    });
    ...
    if (rng.chance(clamp(accept, 0, 0.97))) {
      ...
      virtualShelf.set(sku.id, (virtualShelf.get(sku.id) || 0) - 1);
      revenue += price;
  ...
  if (revenue > 0) {
    const revenuePosting = {
      direction: 'revenue',
      lineKey: 'shopSales',
      category: 'shopSales',
      amount: revenue,
      idempotencyKey: salesPostingId,
    ...
    const revenueReady = preflightLedgerEntry(state, revenuePosting);
    const cogsReady = costOfGoods > 0 ? preflightLedgerEntry(state, cogsPosting) : { ok: true, duplicate: false };
    if (!revenueReady.ok || revenueReady.duplicate || !cogsReady.ok || cogsReady.duplicate) {
      shop.lastAccrualError = revenueReady.reason || cogsReady.reason
        || 'Passive shop sale already has a ledger checkpoint.';
      return {
        ok: false,
        dayAbs,
    ...
    for (const sale of salesPlan) {
      quantities.set(sale.sku.id, (quantities.get(sale.sku.id) || 0) + 1);
    }
    const inventoryPlan = [];
    for (const [skuId, quantity] of quantities) {
      ...
      if (!allocation.ok) {
        shop.lastAccrualError = allocation.reason;
        return {
          ok: false,
          dayAbs,
      ...
      inventoryPlan.push({ skuId, quantity, allocations: allocation.allocations });
    }

    let committedMovements = 0;
    for (const movement of inventoryPlan) {
      const moved = moveInventory(state, {
        ...
        referenceId: `shop-simulation:${dayAbs}:inventory:${movement.skuId}`,
        reason: `Passive pro-shop sales for day ${dayAbs}`,
        refreshOrder: false,
      });
      if (!moved.ok) {
        shop.lastAccrualError = moved.reason;
        return {
          ok: false,
          dayAbs,
          reason: moved.reason,
          inventoryChanged: committedMovements > 0,
          ledgerChanged: false,
        };
      }
      shop.inventory[movement.skuId].shelf -= movement.quantity;
      recordSale(state, movement.skuId, movement.quantity);
      committedMovements++;
    }

    for (const sale of salesPlan) {
      if (sale.member) {
        sale.member.satisfaction = clamp(
          sale.member.satisfaction + (sale.category === 'clubs' ? 3 : 0.6),
          0,
          100,
        );
      }
      if (sale.log) {
        shop.log.unshift(`${sale.member ? sale.member.name : 'A visitor'} bought the ${sale.sku.name} (${Math.round(sale.price)} dollars)`);
        if (shop.log.length > 8) shop.log.pop();
      }
    }

    const bookedRevenue = addRevenue(state, 'shopSales', revenue, revenuePosting);
    if (!bookedRevenue.ok || bookedRevenue.duplicate) {
      shop.lastAccrualError = bookedRevenue.reason || 'Passive sales revenue was not posted.';
      return { ok: false, dayAbs, reason: shop.lastAccrualError, inventoryChanged: true, ledgerChanged: false };
    }
    if (costOfGoods > 0) {
      const bookedCogs = addCostOfGoods(state, costOfGoods, cogsPosting);
      if (!bookedCogs.ok || bookedCogs.duplicate) {
        shop.lastAccrualError = bookedCogs.reason || 'Passive sales cost was not posted.';
        return { ok: false, dayAbs, reason: shop.lastAccrualError, inventoryChanged: true, ledgerChanged: true };
      }
    }
    delete shop.lastAccrualError;
    recordOutcome(state, {
