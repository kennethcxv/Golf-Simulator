...
canvas.addEventListener('click', () => {
  ...
  if (regActive() || toolWheel?.isOpen() || isPauseOpen()) return;
...
canvas.addEventListener('pointerdown', (e) => {
  ...
  if (toolWheel?.isOpen() || isPauseOpen()) return;
...
function updateWalkOverlay() {
  ...
  const label = ['walk', 'placement'].includes(mode)
    ...
    : null;
  setPromptText(prompt, label || '');
  const promptVisible = !!label && !toolWheel?.isOpen();
  if (prompt.dataset.visible !== String(promptVisible)) {
    prompt.dataset.visible = String(promptVisible);
    prompt.style.opacity = promptVisible ? '1' : '0';
  }
  ...
  if (lockHint.dataset.visible !== String(showLockHint)) {
    lockHint.dataset.visible = String(showLockHint);
    lockHint.style.display = showLockHint ? '' : 'none';
  }
  const lockText = learned
    ? 'Click to resume looking'
    : 'Click to look · WASD move · E interact · tap/hold F tools · P pause';
  setPromptText(lockHint, lockText);
  ...
  if (inside && mode === 'walk' && app.state && app.state.shop) {
    ...
    if (cond.textContent !== conditionText) cond.textContent = conditionText;
    cond.style.display = '';
...
function boot() {
  ...
  walkOverlay = el('div', { class: 'shop-overlay', style: 'display:none' },
    ...
    el('div', { class: 'shop-lockhint', text: 'Click to look · WASD move · E interact · tap/hold F tools · P pause' }),
  );
  ...
  gameUi.append(hud.root, worksPanel.palette, worksPanel.planBar, inspectPanel.root, groundsPanel.root, clubPanel.root, empirePanel.root, walkOverlay, regHint, laptopUi.root, objectivesPanel.root, viewToggle, toolWheel.root,
