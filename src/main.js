...
function startGame(state) {
  ...
  app.scene3d.walk.hooks.repairAt = (cx, cy, dtSec, wx, wz) => {
    const hero = heroReportAt(cx, cy, wx, wz);
    if (!hero) return { ok: false, reason: 'The divot kit is assigned to the highlighted hole.' };
  ...
  app.scene3d.walk.hooks.divotLabelAt = (cx, cy, wx, wz) => {
    const hero = heroReportAt(cx, cy, wx, wz);
    if (!hero) return 'Divot kit · assigned to Hole 4 · [F] next';
  ...
  app.scene3d.walk.hooks.rakeAt = (cx, cy, dtSec, wx, wz, directionRad = 0) => {
    const hero = heroReportAt(cx, cy, wx, wz);
    if (!hero) return { ok: false, reason: 'The rake is assigned to Hole 4.' };
  ...
  app.scene3d.walk.hooks.rakeLabelAt = (cx, cy, wx, wz) => {
    const hero = heroReportAt(cx, cy, wx, wz);
    if (!hero) return 'Bunker rake · assigned to Hole 4 · [F] next';
  ...
  app.scene3d.walk.hooks.mowAt = (cx, cy, options = {}) => {
    ...
    if (maintenanceIndexAtWorld(app.state.courseMaintenance, point.x, point.z) < 0) {
      return { ok: false, changed: 0, reason: 'The selected mower is assigned to Hole 4.' };
    }
...
function updateWalkOverlay() {
  ...
  lockHint.style.display = document.pointerLockElement || cursorPanelOpen || workingMaintenanceRoute ? 'none' : '';
...
function boot() {
  ...
  gameUi.append(hud.root, worksPanel.palette, worksPanel.planBar, inspectPanel.root, groundsPanel.root, clubPanel.root, empirePanel.root, maintenancePanel.root, walkOverlay, regHint, laptopUi.root, objectivesPanel.root, viewToggle,
