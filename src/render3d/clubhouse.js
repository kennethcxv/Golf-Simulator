...
export function makeClubhouse(ctx) {
  ...
  const goodsCarryPalmGeo = new THREE.SphereGeometry(0.048, 12, 8);
  ...
  function makeCarryHands(span, y, z) {
    ...
    for (const sign of [-1, 1]) {
      const palm = new THREE.Mesh(goodsCarryPalmGeo, carrySkinMat);
      palm.scale.set(1.05, 0.78, 1.18);
      palm.position.set(sign * span, y, z);
      const thumb = new THREE.Mesh(goodsCarryPalmGeo, carrySkinMat);
  ...
  const carryModelBySku = { polo1: 'polo_folded', polo2: 'polo_folded', jacket2: 'polo_folded', cap1: 'cap', glove1: 'glove', bag1: 'bag' };

  function customerProductVisual(sku, compact = false) {
    const group = new THREE.Group();
    const modelName = sku && carryModelBySku[sku.id];
