...
export function makeClubhouse(ctx) {
  ...
  function updateDoors(dt, now) {
    for (const d of doors) {
      ...
      let near = false;
      if (walk.active && Math.hypot(walk.x - d.world.x, walk.z - d.world.z) < 2.2) near = true;
      let custNear = false;
  ...
  function backshelfUnit(f) {
    const g = new THREE.Group();
    const frame = new THREE.Mesh(new THREE.BoxGeometry(3.2, 2.3, 0.1), darkMat);
    frame.position.set(0, 1.15, -0.28);
    g.add(frame);
    for (const y of [0.4, 1.05, 1.7]) {
      const board = new THREE.Mesh(new THREE.BoxGeometry(3.2, 0.06, 0.6), woodMat);
      board.position.set(0, y, 0);
      g.add(board);
    }
    const swap = Math.abs(Math.sin(f.ry)) > 0.5;
    addCol(colBoxAt(f.x, f.z, swap ? 0.9 : 3.4, swap ? 3.4 : 0.9));
    return g;
  }
  ...
  const office = { computerProp: null };
  {
    ...
    // the interim computer (a laptop replaces this in the next step)
    ...
    monitor.position.set(OFFICE.laptop.x, 0.96, OFFICE.laptop.z);
    monitor.rotation.y = OFFICE.laptop.ry;
    interior.add(monitor);
    const compWp = L2W(OFFICE.laptop.x, OFFICE.laptop.z);
    office.computerProp = addProp({
      x: compWp.x, z: compWp.z, r: 2.2,
      label: () => 'Office computer — [E] supplier orders & pricing',
      action: () => { if (hooks.openShopDesk) hooks.openShopDesk(); },
    });
  ...
  function rebuildStock() {
    ...
    for (const f of FIXTURES) {
      ...
      if (f.kind === 'backshelf') {
        ...
        for (let i = 0; i < show; i++) {
          const bx = -1.35 + (i % 5) * 0.62;
          const by = [0.46, 1.11, 1.76][Math.floor(i / 5) % 3];
  ...
  function dispose() {
    scene.remove(group, interior, custGroup, motes);
