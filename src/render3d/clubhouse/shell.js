...
export function buildShell(B) {
  ...
  function addCan(lx, lz, { real = false, intensity = 9 } = {}) {
    ...
    const disc = new THREE.Mesh(
      ...
      new THREE.MeshStandardMaterial({ color: generatedLightColor, emissive: generatedLightColor, emissiveIntensity: 1.25 }),
    );
  ...
  function addLantern(lx, lz) {
    ...
    const light = new THREE.PointLight(generatedLightColor, 11.5, 9.5, 1.7);
    light.position.set(lx, CEIL_Y - 0.85, lz);
    interior.add(light);

    const glassMat = new THREE.MeshStandardMaterial({
      color: generatedLightColor, emissive: generatedLightColor, emissiveIntensity: 1.35,
    });
