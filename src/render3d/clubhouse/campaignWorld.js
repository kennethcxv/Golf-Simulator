...
import { COUNTER, HOURS_SIGN, OFFICE, REGISTER, STOCKROOM } from '../../data/shopLayout.js';

const FACILITY_SITES = Object.freeze([
  { id: 'officeDesk', x: OFFICE.desk.x - 0.55, z: OFFICE.desk.z + 0.45, w: 1.55, d: 0.75 },
  { id: 'officeChair', x: OFFICE.chair.x, z: OFFICE.chair.z - 0.52, w: 0.72, d: 0.72 },
  { id: 'laptop', x: OFFICE.laptop.x - 0.55, z: OFFICE.laptop.z - 0.38, w: 0.65, d: 0.45 },
...
export function buildCampaignWorld(B, { refreshWorld = () => {} } = {}) {
  ...
  for (const site of FACILITY_SITES) {
    const marker = outlineMarker(site.w, site.d);
    marker.position.set(site.x, 0.018, site.z);
    root.add(marker);
