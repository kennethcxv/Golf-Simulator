...
export function createRegisterMode(B) {
  ...
  function customerGripPoint(side = 'R') {
    ...
    const grip = char && typeof char.carryGrip === 'function' ? char.carryGrip(side) : null;
    if (!grip) return null;
    root.updateMatrixWorld(true);
  ...
  function begin(customer) {
    ...
    refillDrawerMoney();
    scheduleCheckoutTexturePrewarm();
    // Queue handoff can assign the next customer while the player remains in
    // register mode. enter() only runs for the first customer, so explicitly
    // arm the same cashier-entry transition for every fresh transaction owned
    // by an already-active till. Without this, domain stages advance while the
    // checkout flow stays WaitingForCashier and the completed handoff cannot
    // legally reach CustomerLeaving or bank the sale.
    if (active && checkoutFlowState() === 'WaitingForCashier') {
      enterTimer = 0.30;
      flowTo('EnteringCashierMode', 'next-queued-customer-entered-active-register');
    }
    drawScreen();
    drawTerm();
    return true;
