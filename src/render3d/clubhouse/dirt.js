...
export function buildDirt(B, windowDefs) {
  ...
  function buildPlan() {
    ...
    const registerScuff = frontDeskPoint(-1.2, -1.1);
    const queueScuff = frontDeskPoint(0.4, -1.6);
    scuff(registerScuff.x, registerScuff.z, 8, 31);
    scuff(queueScuff.x, queueScuff.z, 6, 57);
  ...
  windowDefs.forEach((wd, i) => {
    ...
    film.name = `LegacyWindowDirtFilm_${i}`;
    film.userData.legacyPresentation = 'window-dirt-film';
    ...
    addProp({
      ...
      action: () => {
        ...
        if (!res.ok) return;
        if (B.presentRestorationFeedback) B.presentRestorationFeedback(res.restoration);
        refreshFilms();
