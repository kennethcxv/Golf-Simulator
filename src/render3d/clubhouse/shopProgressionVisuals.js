...
import { COUNTER, INTERIOR, STOCKROOM, frontDeskPoint } from '../../data/shopLayout.js';
import { SHOP_TIERS, shopTier, shopTierIndex } from '../../sim/shopProgression.js';
import { makeSignTexture } from './materials.js';

export function futureLoungeBarrierVisible(state, tierIndex = shopTierIndex(state)) {
  const furnishedCampaign = Number(state?.campaign?.furnishedStartVersion) > 0;
  return tierIndex < 2 && !furnishedCampaign;
}
...
export function buildShopProgressionVisuals(B) {
  ...
  const fasciaPosition = frontDeskPoint(0, -COUNTER.depth / 2 - 0.019);
  fascia.position.set(fasciaPosition.x, 0.54, fasciaPosition.z);
  fascia.rotation.y = COUNTER.ry;
  ...
  const fasciaTrimPosition = frontDeskPoint(0, -COUNTER.depth / 2 - 0.04);
  fasciaTrim.position.set(fasciaTrimPosition.x, 0.84, fasciaTrimPosition.z);
  fasciaTrim.rotation.y = COUNTER.ry;
  ...
  const insetPosition = frontDeskPoint(0, -COUNTER.depth / 2 - 0.055);
  counterInset.position.set(insetPosition.x, 0, insetPosition.z);
  counterInset.rotation.y = COUNTER.ry;
  const insetPanel = new THREE.Mesh(new THREE.BoxGeometry(1.42, 0.28, 0.026), mats.greenPaint);
  insetPanel.position.set(0, 0.54, 0);
  ...
  function refresh() {
    ...
    setBarrierActive(futureLoungeBarrierVisible(state, index));
