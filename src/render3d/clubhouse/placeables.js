...
export function buildPlaceables(B, { fixtureAnchors, fallbackWelcomeMat = null } = {}) {
  ...
  function rebuild() {
    ...
    const wanted = new Map(objects.filter((object) => (
      object.render?.kind === 'glb'
      && object.render?.owner !== 'unified-61-100-runtime'
  ...
  return {
    ...
    releasePreview,
    diagnostics: () => ({
      expectedPlacedModels: placedObjects(state).filter((object) => (
        object.render?.kind === 'glb'
        && object.render?.owner !== 'unified-61-100-runtime'
      )).length,
      ...
      externallyOwnedModels: placedObjects(state).filter((object) => (
        object.render?.owner === 'unified-61-100-runtime'
      )).map((object) => object.assetNumber),
    }),
    dispose() {
      if (disposed) return;
      disposed = true;
