...
// Characters are numerous (shopper, course walkers, rental-cart parties, and
// vehicle drivers), but their wardrobe is a small authored palette and their
// rigid body pieces use the same handful of meshes. Reusing those immutable
// resources cuts hundreds of duplicate GPU allocations. Reference counts keep
// the existing disposal contract honest: a resource is released only after the
// final live character using it goes away.
const GEOMETRY_CACHE = new Map();
const MATERIAL_CACHE = new Map();
const RESOURCE_REFS = new WeakMap();

function sharedGeometry(key, make) {
  if (!GEOMETRY_CACHE.has(key)) GEOMETRY_CACHE.set(key, make());
...
const M = (color, rough = 0.85) => {
  ...
  if (!MATERIAL_CACHE.has(key)) {
    MATERIAL_CACHE.set(key, new THREE.MeshStandardMaterial({ color: hex, roughness: rough }));
  }
  return MATERIAL_CACHE.get(key);
};

function retainCharacterResource(resource) {
  RESOURCE_REFS.set(resource, (RESOURCE_REFS.get(resource) || 0) + 1);
}

function releaseCharacterResource(resource) {
  const next = Math.max(0, (RESOURCE_REFS.get(resource) || 1) - 1);
  RESOURCE_REFS.set(resource, next);
  if (next === 0) resource.dispose();
}

function box(w, h, d, mat, y = 0, z = 0) {
  const m = new THREE.Mesh(sharedGeometry(`box:${w}:${h}:${d}`, () => new THREE.BoxGeometry(w, h, d)), mat);
...
function ellipsoid(w, h, d, mat, y = 0, z = 0, segments = 14) {
  const rows = Math.max(10, segments - 2);
  const m = new THREE.Mesh(
    sharedGeometry(`sphere-unit:${segments}:${rows}`, () => new THREE.SphereGeometry(0.5, segments, rows)),
    mat,
  );
  m.scale.set(w, h, d);
  m.position.set(0, y, z);
...
function capsule(radius, straight, mat, y = 0) {
  ...
  const m = new THREE.Mesh(
    sharedGeometry(`capsule:${radius}:${straight}`, () => new THREE.CapsuleGeometry(radius, straight, 6, 14)),
    mat,
  );
  m.position.y = y;
  m.castShadow = true;
...
function ball(radius, mat, segments = 16) {
  const rows = Math.max(10, segments - 4);
  const m = new THREE.Mesh(
    sharedGeometry(`sphere:${radius}:${segments}:${rows}`, () => new THREE.SphereGeometry(radius, segments, rows)),
    mat,
  );
  m.castShadow = true;
  return m;
...
export function makeCharacter({ polo = 0x3b6fb3, khaki = 0xc2b190, cap = 0xf2efe4, skin = 0xd9a97e } = {}) {
  ...
  const belt = new THREE.Mesh(
    sharedGeometry('belt-cylinder', () => new THREE.CylinderGeometry(0.205, 0.198, 0.038, 18)),
    mBelt,
  );
  ...
  const torso = new THREE.Mesh(sharedGeometry('polo-lathe', () => new THREE.LatheGeometry([
  ...
  const yoke = new THREE.Mesh(
    sharedGeometry('polo-yoke', () => new THREE.CapsuleGeometry(0.135, 0.26, 6, 14)),
    mPolo,
  );
  ...
  const neck = new THREE.Mesh(
    sharedGeometry('character-neck', () => new THREE.CylinderGeometry(0.058, 0.076, 0.18, 16)),
    mSkin,
  );
  ...
  const collar = new THREE.Mesh(
    sharedGeometry('polo-collar', () => new THREE.CylinderGeometry(0.108, 0.072, 0.058, 18, 1, true)),
    mPolo,
  );
  ...
  for (const by of [0.34, 0.26]) {
    const button = new THREE.Mesh(
      sharedGeometry('polo-button', () => new THREE.SphereGeometry(0.010, 8, 6)),
      M(0xf3efe6, 0.5),
    );
  ...
  const skull = new THREE.Mesh(
    sharedGeometry('character-skull', () => new THREE.SphereGeometry(0.155, 20, 14)),
    mSkin,
  );
  ...
  for (const x of [-0.057, 0.057]) {
    ...
    const eye = new THREE.Mesh(
      sharedGeometry('character-eye', () => new THREE.SphereGeometry(0.0165, 10, 7)),
      mFace,
    );
    ...
    const catchlight = new THREE.Mesh(
      sharedGeometry('character-eye-catchlight', () => new THREE.SphereGeometry(0.0045, 7, 5)),
      mEyeLight,
    );
  ...
  if (mCap) {
    ...
    const crown = new THREE.Mesh(
      sharedGeometry('golf-cap-crown', () => new THREE.SphereGeometry(0.168, 20, 12, 0, Math.PI * 2, 0, Math.PI * 0.58)),
      mCap,
    );
    ...
    head.add(crown);
    const button = new THREE.Mesh(
      sharedGeometry('golf-cap-button', () => new THREE.SphereGeometry(0.016, 8, 6)),
      mCap,
    );
    button.position.y = 0.196;
    head.add(button);
    // curved bill: a flattened, slightly down-tilted disc projecting from the crown front
    const bill = ellipsoid(0.170, 0.024, 0.135, mCap, 0.105, 0.135, 16);
    bill.rotation.x = 0.17;
    head.add(bill);
  } else {
    // bare head gets hair instead of a cap
    const hair = new THREE.Mesh(
      sharedGeometry('character-hair', () => new THREE.SphereGeometry(0.16, 12, 8, 0, Math.PI * 2, 0, Math.PI / 2.1)),
      M(0x4a3a28, 0.95),
    );
    hair.position.y = 0.1;
    head.add(hair);
  ...
  for (const [side, sx] of [['L', 1], ['R', -1]]) {
    ...
    const sleeveCuff = new THREE.Mesh(
      sharedGeometry('polo-sleeve-cuff', () => new THREE.CylinderGeometry(0.068, 0.062, 0.03, 14)),
      mPolo,
    );
  ...
  for (const geometry of ownedGeometries) retainCharacterResource(geometry);
  for (const material of ownedMaterials) retainCharacterResource(material);
  ...
  const char = {
    ...
    idleVariant: ((polo ^ khaki ^ (cap ?? 0x4a3524)) >>> 0) % 3,
    transition: null,
    equipment: null,
    dispose() {
      if (resourcesDisposed) return false;
      resourcesDisposed = true;
      for (const geometry of ownedGeometries) releaseCharacterResource(geometry);
      for (const material of ownedMaterials) releaseCharacterResource(material);
      return true;
    },
  };

  const capturePose = () => ({
    hipL: limbs.hipL.rotation.x,
    hipR: limbs.hipR.rotation.x,
    kneeL: limbs.kneeL.rotation.x,
    kneeR: limbs.kneeR.rotation.x,
    shL: limbs.shoulderL.rotation.x,
    shR: limbs.shoulderR.rotation.x,
    shLz: limbs.shoulderL.rotation.z,
    shRz: limbs.shoulderR.rotation.z,
    elbL: limbs.elbowL.rotation.x,
    elbR: limbs.elbowR.rotation.x,
    lean: chest.rotation.x,
    twist: chest.rotation.y,
    headTilt: head.rotation.x,
    headYaw: head.rotation.y,
    chestY: chest.position.y,
    pelvisY: pelvis.position.y,
    hipLY: limbs.hipL.position.y,
    hipRY: limbs.hipR.position.y,
  });

  char.setMode = (mode) => {
    if (char.mode !== mode) {
      char.transition = { from: capturePose(), elapsed: 0, duration: 0.24 };
      char.mode = mode;
      char.phase = 0;
    }
  };
  ...
  char.attachEquipment = (object, { side = 'R', kind = 'generic' } = {}) => {
    if (!object) return null;
    if (char.equipment?.object?.parent) char.equipment.object.parent.remove(char.equipment.object);
    const grip = char.carryGrip(side);
    grip.add(object);
    object.position.set(0, -0.012, 0.008);
    object.rotation.set(0.04, 0, side === 'R' ? -0.10 : 0.10);
    char.equipment = { object, side, kind };
    return object;
  };
  char.detachEquipment = () => {
    const object = char.equipment?.object || null;
  ...
  char.update = (dt) => {
    ...
    let lean = 0.04, twist = 0, headTilt = 0, headYaw = 0, bob = 0, shLz = 0.06, shRz = -0.06;
    ...
    if (char.mode === 'Walk' || char.mode === 'WalkBag') {
      ...
      // A golf swing is a two-handed action: converge the shoulder chains so
      // both palms read as one grip instead of two unrelated reaching arms.
      shLz = -0.50;
      shRz = 0.50;
    ...
    } else { // Idle — three restrained, outfit-stable variations
      const breath = Math.sin(p * 1.1);
      lean = 0.03 + 0.013 * breath;
      bob = 0.008 * breath;
      if (char.idleVariant === 1) {
        // Weight settled on one foot with a slow glance toward the next tee.
        shL = 0.035 + 0.018 * breath;
        shR = 0.075 + 0.022 * Math.sin(p * 1.1 + 0.45);
        shLz = 0.025;
        shRz = -0.095;
        twist = 0.025 * Math.sin(p * 0.47);
        headYaw = 0.11 * Math.sin(p * 0.38);
      } else if (char.idleVariant === 2) {
        // Hands loose at the sides while the head quietly surveys the course.
        shL = 0.08 + 0.020 * breath;
        shR = 0.045 + 0.018 * Math.sin(p * 1.1 + 0.7);
        shLz = 0.085;
        shRz = -0.035;
        headTilt = 0.025 + 0.018 * Math.sin(p * 0.62);
        headYaw = -0.14 * Math.sin(p * 0.31);
      } else {
        shL = 0.06 + 0.03 * breath;
    ...
    const target = {
      hipL, hipR, kneeL, kneeR, shL, shR, shLz, shRz,
      elbL: elb, elbR: elb, lean, twist, headTilt, headYaw,
      chestY: 1.07 + bob,
      pelvisY: 0.98 + bob * 0.7,
      hipLY: 0.93 + sitOffset,
      hipRY: 0.93 + sitOffset,
    };
    let pose = target;
    if (char.transition) {
      char.transition.elapsed += Math.max(0, dt);
      const raw = Math.min(1, char.transition.elapsed / char.transition.duration);
      const eased = raw * raw * (3 - 2 * raw);
      pose = {};
      for (const key of Object.keys(target)) {
        pose[key] = THREE.MathUtils.lerp(char.transition.from[key], target[key], eased);
      }
      if (raw >= 1) char.transition = null;
    }

    limbs.hipL.rotation.x = pose.hipL;
    limbs.hipR.rotation.x = pose.hipR;
    limbs.hipL.position.y = pose.hipLY;
    limbs.hipR.position.y = pose.hipRY;
    limbs.kneeL.rotation.x = pose.kneeL;
    limbs.kneeR.rotation.x = pose.kneeR;
    limbs.shoulderL.rotation.x = pose.shL;
    limbs.shoulderR.rotation.x = pose.shR;
    limbs.shoulderL.rotation.z = pose.shLz;
    limbs.shoulderR.rotation.z = pose.shRz;
    limbs.elbowL.rotation.x = pose.elbL;
    limbs.elbowR.rotation.x = pose.elbR;
    chest.rotation.x = pose.lean;
    chest.rotation.y = pose.twist;
    head.rotation.x = pose.headTilt;
    head.rotation.y = pose.headYaw;
    chest.position.y = pose.chestY; // bob lives on the body — root stays placeable
    pelvis.position.y = pose.pelvisY;

    if (char.equipment?.kind === 'golf-club') {
      const { object } = char.equipment;
      const swingModes = ['Swing', 'DriverSwing', 'IronSwing', 'PracticeSwing', 'BunkerSwing'];
      const isSwingMode = swingModes.includes(char.mode);
      object.visible = char.mode === 'Idle' || char.mode === 'Walk' || isSwingMode;
      const swingTime = p % (char.mode === 'PracticeSwing' ? 2.9 : 2.6);
      const roll = isSwingMode
        ? lerpSeg(swingTime, [[0, -0.12], [0.7, -0.12], [1.2, 2.20], [1.45, -1.55], [2.1, -0.12], [2.6, -0.12]])
        : char.mode === 'Walk' ? -0.08 : -0.12;
      const pitch = isSwingMode
        ? lerpSeg(swingTime, [[0, 0.04], [0.7, 0.04], [1.2, 0.30], [1.45, -0.20], [2.1, 0.04], [2.6, 0.04]])
        : 0.04;
      const equipmentEase = Math.min(1, Math.max(0, dt) * 12);
      object.rotation.x = THREE.MathUtils.lerp(object.rotation.x, pitch, equipmentEase);
      object.rotation.z = THREE.MathUtils.lerp(object.rotation.z, roll, equipmentEase);
    }
  };
