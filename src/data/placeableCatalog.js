...
import {
  BACKDOOR_CLEARWAY, COUNTER, COUNTER_TOP, DOOR_BACK, DOOR_CLEARWAY, DOOR_MAIN, DOOR_STOCK,
  FIXTURES, FIXTURE_HALF, FRONT_DESK, FRONT_DESK_ASSETS, FRONT_DESK_FRAME, INTERIOR,
  OFFICE, REGISTER, SHELL, STOCKROOM, WINDOW_DIM, WINDOWS, frontDeskRect,
} from './shopLayout.js';
import { PRO_SHOP_FURNITURE_PLACEABLES } from './proShopFurniture.js';
...
const fixtureRecords = FIXTURES.map((fixture) => {
  ...
  const footprint = fixture.footprint || null;
  const width = footprint
    ? footprint.maxX - footprint.minX
    : (fixture.short ? 0.85 : half[0]) * 2;
  const depth = footprint ? footprint.maxZ - footprint.minZ : half[1] * 2;
  const footprintVolumes = footprint ? [{
  ...
  return {
    ...
    bounds: bounds(width, profile.height, depth, footprintVolumes),
...
export const FRONT_DESK_RUNTIME_ASSET_NUMBERS = Object.freeze([61, 81, 83, 85, 88, 89, 90]);
const FRONT_DESK_RUNTIME_ASSET_SET = new Set(FRONT_DESK_RUNTIME_ASSET_NUMBERS);

const DEFAULTS = {
  61: transform(FRONT_DESK_ASSETS.asset61.x, 0, FRONT_DESK_ASSETS.asset61.z,
  ...
  87: transform(INTERIOR.w / 2 - 0.06, 2.25, 2.65, -Math.PI / 2, 'wall', { wallId: 'east-office', normal: [-1, 0, 0], socket: 'SOCKET_WallMount' }, 'office'),
  88: transform(FRONT_DESK.keyRack.x, 1.52, FRONT_DESK.keyRack.z,
    FRONT_DESK.keyRack.ry, 'wall', { wallId: 'front-desk-backdrop', normal: [0, 0, 1], socket: 'SOCKET_KeyRack' }, 'sales'),
  89: transform(FRONT_DESK.clipboard.x, COUNTER_TOP, FRONT_DESK.clipboard.z,
    FRONT_DESK.clipboard.ry, 'counter', { parentId: 'surface:checkout-counter', socket: 'SOCKET_Clipboard' }, 'sales'),
  90: transform(FRONT_DESK.scorecards.x, COUNTER_TOP, FRONT_DESK.scorecards.z,
    FRONT_DESK.scorecards.ry, 'counter', { parentId: 'surface:checkout-counter', socket: 'SOCKET_Scorecards' }, 'sales'),
  91: transform(5.64, 1.02, -0.85, Math.PI / 2, 'wall', { wallId: 'partition-sales', normal: [-1, 0, 0], socket: 'SOCKET_WallMount' }, 'sales'),
  92: transform(8.55, 2.60, -INTERIOR.d / 2 + 0.06, 0, 'wall', { wallId: 'north-stockroom', normal: [0, 0, 1], socket: 'SOCKET_WallMount' }, 'stockroom'),
  ...
  99: transform(0.75, 0, 5.00, 0.30, 'floor', null, 'sales'),
  100: transform(DOOR_MAIN.x, 0.004, 5.05, 0, 'floor', null, 'sales'),
};
...
const assetRecords = ASSET_DEFS.map(([number, stem, label, dimsM, allowed, category, value]) => {
  ...
  const frontDeskRuntimeOwned = FRONT_DESK_RUNTIME_ASSET_SET.has(number);
  const noNav = wallOnly || small || number === 99 || number === 100 || frontDeskRuntimeOwned;
  const defaultTransform = DEFAULTS[number] || transform(0, 0, 0, 0, allowed[0], null, 'storage');
  const requiredObject = number === 94 ? 'safety-critical'
    : (frontDeskRuntimeOwned ? 'front-desk-runtime' : false);
  return {
    id,
    ...
    render: {
      ...
      owner: frontDeskRuntimeOwned ? 'unified-61-100-runtime' : 'placeable-runtime',
    },
    ...
    requiredObject,
    placementRestrictions: frontDeskRuntimeOwned
      ? ['must-remain-placed', 'authored-socket-only', 'front-desk-runtime']
      : (requiredObject ? ['must-remain-placed', 'door-safety'] : []),
...
const protectedRecords = [
  {
    ...
    bounds: bounds(COUNTER.len, 0.96, COUNTER.depth, [
      { x: 0, z: 0, width: FRONT_DESK_FRAME.frontLength, depth: FRONT_DESK_FRAME.frontDepth },
      {
        x: -FRONT_DESK_FRAME.frontLength / 2 + FRONT_DESK_FRAME.returnCollisionWidth / 2,
        z: (FRONT_DESK_FRAME.frontDepth / 2 + FRONT_DESK_FRAME.returnStaffExtent) / 2,
        width: FRONT_DESK_FRAME.returnCollisionWidth,
        depth: FRONT_DESK_FRAME.returnStaffExtent - FRONT_DESK_FRAME.frontDepth / 2,
      },
    ]),
    collision: { mode: 'analytic', blocksPlayer: true, blocksCustomers: true },
    ...
    defaultState: 'placed', defaultTransform: transform(COUNTER.x, 0, COUNTER.z, COUNTER.ry, 'floor', { socket: 'SOCKET_CHECKOUT_ANCHOR' }, 'sales'),
  },
  {
    id: 'core-laptop', assetId: 'CORE_OFFICE_LAPTOP', label: 'Pine Hills tee-sheet laptop',
    placementCategory: 'required-hardware', source: 'front-desk',
    render: { kind: 'existing', anchorKey: 'front-desk-laptop' },
    ...
    defaultState: 'placed', defaultTransform: transform(FRONT_DESK.laptop.x, COUNTER_TOP, FRONT_DESK.laptop.z, FRONT_DESK.laptop.ry, 'counter', { parentId: 'surface:checkout-counter', socket: 'SOCKET_Laptop' }, 'sales'),
  },
];
...
export const PLACEABLES = deepFreeze([
  ...fixtureRecords, ...assetRecords, ...PRO_SHOP_FURNITURE_PLACEABLES,
  ...protectedRecords, ...counterHardwareRecords,
]);
...
export const WALL_SURFACES = deepFreeze([
  { id: 'front-desk-backdrop', axis: 'z', at: FRONT_DESK.keyRack.z, from: FRONT_DESK.logoBackdrop.x - 1.15, to: FRONT_DESK.keyRack.x + 1.15, coordinate: 'x', normal: [0, 0, 1], yaw: FRONT_DESK.keyRack.ry, room: 'sales', exterior: false },
...
const counterSurface = {
  ...
  x: COUNTER.x, z: COUNTER.z, ry: COUNTER.ry, width: COUNTER.len - 0.12, depth: COUNTER.depth - 0.10,
  sockets: [
    { id: 'SOCKET_Laptop', ...FRONT_DESK.laptop },
    { id: 'SOCKET_Phone', ...FRONT_DESK.phone },
    { id: 'SOCKET_DeskLamp', ...FRONT_DESK.deskLamp },
    { id: 'SOCKET_Clipboard', ...FRONT_DESK.clipboard },
    { id: 'SOCKET_Scorecards', ...FRONT_DESK.scorecards },
  ],
  protectedZones: [
...
export const PROTECTED_ZONES = deepFreeze([
  { id: 'main-door', label: 'main entrance', ...DOOR_CLEARWAY, critical: true },
