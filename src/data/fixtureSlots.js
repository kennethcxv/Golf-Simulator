...
// Every product line owns a real column or lane on its fixture. This makes
// capacity, rendering, stocking, and save recovery share the same addresses.
...
// The Pine Hills opening cooler is authored as four shelves of six bottles.
// Each SKU owns two adjacent facings per shelf (8 units total), matching the
// simulation-owned cooler contract.  The named sockets are authoritative once
// the GLB is present; the metric poses keep the older fixture usable as a
// deterministic fallback while that model is loading.
function openingCoolerLane(skuId) {
  const lane = ['water1', 'sportdrink2', 'soda1'].indexOf(skuId);
  if (lane < 0) return [];
  const metresToYards = 1 / 0.9144;
  const xs = [-0.30, -0.18, -0.06, 0.06, 0.18, 0.30];
  const heights = [0.36, 0.70, 1.04, 1.38];
  return heights.flatMap((y, shelf) => [0, 1].map((facing) => {
    const column = lane * 2 + facing;
    const socketIndex = shelf * 6 + column + 1;
    return {
      x: xs[column] * metresToYards,
      y: y * metresToYards,
      z: 0.05,
      ry: 0,
      socketName: `SOCKET_Bottle_${String(socketIndex).padStart(2, '0')}`,
    };
  }));
}
...
const BUILD = {
  ...
  // Asset 20 owns named hanger and fold sockets; keep that authored eight-slot
  // contract instead of treating the freestanding rail as a generic wall bay.
  jacket2: apparelWall,
  cap1: hatLane,
  cap2: hatLane,
  visor1: hatLane,
  ...
  // Opening cooler: 24 authored bottle sockets, eight per existing SKU.
  water1: openingCoolerLane,
  sportdrink2: openingCoolerLane,
  soda1: openingCoolerLane,
