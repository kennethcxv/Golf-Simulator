...
export const PLACEABLE_ITEM_CATALOG = Object.freeze(decor.map((sku) => Object.freeze({
  assetId: sku.placeableAssetId || `shop-decor:${sku.id}`,
  skuId: sku.id,
  displayName: sku.name,
  category: sku.furnitureCategory || CATEGORY_BY_SKU[sku.id] || 'decorations',
  variants: Object.freeze([sku.furnitureTierId || 'standard']),
  defaultVariant: sku.furnitureTierId || 'standard',
  defaultCondition: 100,
  purchasePrice: sku.cost,
  sellValue: Math.round(sku.cost * 0.6 * 100) / 100,
  placementProfile: sku.placeableProfile || PLACEMENT_PROFILE_BY_SKU[sku.id],
  functionalProfile: sku.functionalProfile || null,
  wallSnap: sku.wallSnap || null,
  renderPath: sku.modelPath || null,
  modelLodPaths: sku.modelLodPaths ? Object.freeze([...sku.modelLodPaths]) : null,
  modelLodDistancesM: sku.modelLodDistancesM ? Object.freeze([...sku.modelLodDistancesM]) : null,
  modelScale: sku.modelScale || 1,
  mountYOffset: sku.mountYOffset || 0,
  lightingProfile: sku.lightingProfile || null,
  placementRestrictions: Object.freeze({
    propertyAreas: Object.freeze(['clubhouse']),
    mounts: Object.freeze(sku.placeableProfile ? [sku.placeableProfile.mount] : authoredMounts(sku.id)),
    requiresAuthoredBounds: !sku.placeableProfile,
  ...
  deliveryRequirement: Object.freeze({
    required: true,
    deliveryClass: sku.placeableProfile?.blocksMovement ? 'large-furniture' : (DELIVERY_CLASS_BY_SKU[sku.id] || 'small-fixture'),
    physicalReceiving: true,
  }),
  unlockRequirement: Object.freeze({ shopTier: sku.tier }),
  upgradeTier: sku.furnitureTier || sku.tier,
})));
