...
export const CONSTRUCTION_QUALITY_LEVELS = deepFreeze([
  {
    id: 'municipal', level: 1, label: 'Municipal', multiplier: 1,
    warrantyYears: 1, craftsmanship: 'serviceable contractor installation',
    visual: 'economical profiles, visible fasteners, broad seams and a durable matte finish',
  },
  {
    id: 'standard', level: 2, label: 'Standard', multiplier: 1.45,
    warrantyYears: 3, craftsmanship: 'clean commercial installation',
    visual: 'straighter joins, coordinated hardware, tighter seams and consistent sheen',
  },
  {
    id: 'premium', level: 3, label: 'Premium', multiplier: 2.1,
    warrantyYears: 7, craftsmanship: 'club-grade fitted installation',
    visual: 'thicker stock, eased edges, concealed fixings and richer material variation',
  },
  {
    id: 'high-end', level: 4, label: 'High-end', multiplier: 3.15,
    warrantyYears: 12, craftsmanship: 'bespoke millwork and specialist fitting',
    visual: 'custom proportions, deep profiles, book-matched surfaces and refined metalwork',
  },
  {
    id: 'luxury', level: 5, label: 'Luxury country club', multiplier: 5,
    warrantyYears: 20, craftsmanship: 'heritage country-club fabrication',
    visual: 'hand-finished materials, tailored transitions, restrained brass and presentation-grade detailing',
  },
]);
...
const finish = (id, label, baseRate, description, extra = {}) => ({
  id, label, baseRate, description, ...extra,
});

export const CONSTRUCTION_FINISH_CATEGORIES = deepFreeze([
  category('flooring', 'Flooring', { quantity: 1900, unit: 'sq ft' }, [
  ...
  category('ceilings', 'Ceilings', { quantity: 1900, unit: 'sq ft' }, [
...
export const CONSTRUCTION_FINISH_FAMILY_COUNT = CONSTRUCTION_FINISH_CATEGORIES
  .reduce((sum, entry) => sum + entry.finishes.length, 0);

export const CONSTRUCTION_FINISH_VARIANT_COUNT = CONSTRUCTION_FINISH_FAMILY_COUNT
  * CONSTRUCTION_QUALITY_LEVELS.length;
...
export function constructionFinishFamily(categoryId, finishId) {
  return CONSTRUCTION_CATEGORY_BY_ID[categoryId]?.finishes
    .find((entry) => entry.id === finishId) || null;
}
...
export function constructionFinishVariant(categoryId, finishId, qualityId) {
  ...
  return Object.freeze({
    id: constructionFinishSelectionId(categoryId, finishId, qualityId),
    categoryId,
    categoryLabel: categorySpec.label,
    finishId,
    finishLabel: family.label,
    qualityId,
    ...
    warrantyYears: quality.warrantyYears,
    quantity,
    unit,
    rate,
    cost,
...
export function allConstructionFinishVariants() {
  return CONSTRUCTION_FINISH_CATEGORIES.flatMap((categorySpec) => (
    categorySpec.finishes.flatMap((family) => (
      CONSTRUCTION_QUALITY_LEVELS.map((quality) => (
        constructionFinishVariant(categorySpec.id, family.id, quality.id)
      ))
    ))
  ));
}
