...
## Furniture catalog (2026-07-20)

The complete player furniture catalog is original project-owned work generated by
`tools/blender/build_furniture_catalog.py`; it does not use downloaded, Tripo, marketplace,
or third-party geometry or textures. All three concept images in `Designs/ClubHouse/`
guided the municipal-to-commercial-to-country-club progression, palette, proportions,
and merchandising families. The images are direction references only and are not
embedded in the models or thumbnails.

The exact user-provided, project-internal direction references are:
...
- `tools/blender/furniture_catalog_manifest.json` is the repeatable bridge from the
  immutable game catalog to Blender dimensions, names, tiers, paths, and mount contracts.
- 62 editable five-tier `.blend` sources live in
  `asset_sources/blender/furniture_catalog/`.
- 310 runtime GLBs live in `vendor/models/furniture/catalog/`; each has applied mesh
  transforms, metre-scale dimensions, an authored mount socket, a `COL_` simplified
  collision box, named shared stylized-PBR materials, and metadata recording its source.
- 310 original 320x180 Blender renders live in `vendor/images/furniture/catalog/` and
  are used directly by the in-game purchase cards.
- `tools/blender/validate_furniture_catalog.py` re-imports every export and retains its
  full audit at `qa/furniture_catalog/blender_validation.md` and `.json`.

The palette is warm cream, deep golf green, muted sage, medium walnut, natural oak,
warm charcoal, and restrained brass. All runtime GLBs and editable sources are
`UNLICENSED` project assets.
