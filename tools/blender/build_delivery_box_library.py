...
def product_skus_by_layout():
    """Evaluate the canonical generated JS contract without a second SKU list."""
    script = """
      import { PRODUCT_PACKAGING } from './src/data/productPackaging.js';
      const result = {};
      for (const entry of Object.values(PRODUCT_PACKAGING)) {
        (result[entry.layoutId] ||= []).push(entry.skuId);
      }
      for (const skus of Object.values(result)) skus.sort();
    ...
    if not result:
        raise RuntimeError("productPackaging.js yielded no SKU/layout contracts")
    return {layout_id: tuple(sorted(skus)) for layout_id, skus in result.items()}


PRODUCT_SKUS_BY_LAYOUT = product_skus_by_layout()
...
def layout(layout_id, category, skus, packed_state, slots):
    contracted_skus = PRODUCT_SKUS_BY_LAYOUT.get(layout_id)
    if contracted_skus is None:
        raise RuntimeError(f"layout {layout_id} is missing from productPackaging.js")
    return {
        "id": layout_id,
        "category": category,
        "skus": contracted_skus,
    ...
    "delivery_accessory_carton": {
        ...
        "layouts": (
            layout(
                "ACCESSORY_CARD12",
                "accessories",
                ("tees1", "towel1", "marker1", "divot1", "sunglasses2"),
