...
def validate_asset_scene(
    ...
    *,
    ...
    try:
        ...
        # "Below floor" only means something for an asset that stands on one. A viewmodel
        # hangs from a hand and a wall or ceiling fixture hangs from its mount, so both
        # legitimately extend below their own origin. Warning on those buries the cases
        # where a floor-standing asset really is sunk, which is what this check is for.
        # Non-floor assets declare `mount` on their root.
        floor_standing = str(root.get("mount", "floor")) == "floor"
        first_person_variant = identity is not None and identity.first_person
        if bounds.minimum[2] < -0.01 and floor_standing and not first_person_variant:
