...
from build_delivery_box_library import PRODUCT_SKUS_BY_LAYOUT  # noqa: E402
...
def build_generic_box(M):
    ...
    layout_id = "CAP_NEST8"
    allowed_skus = PRODUCT_SKUS_BY_LAYOUT.get(layout_id)
    if not allowed_skus:
        raise RuntimeError(f"{layout_id} is missing from productPackaging.js")
...
def validate_scene(asset_id, root, required):
    ...
    if asset_id == BOX_ID:
        ...
        if tuple(json.loads(layout.get("allowed_skus", "[]"))) != PRODUCT_SKUS_BY_LAYOUT["CAP_NEST8"]:
