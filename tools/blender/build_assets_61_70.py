...
def build_61() -> bpy.types.Object:
    ...
    _box("CounterTopInlay", (2.70, 0.70, 0.018), (0.0, -0.025, 0.956), p["natural_oak"], details,
...
def build_62() -> bpy.types.Object:
    ...
    _box("WallCabinetCarcass", (3.08, 0.48, 0.92), (0.0, 0.065, 1.61), m["dark_walnut"], carcass,
         bevel=0.022)
    _box("WallCrown", (3.20, 0.54, 0.09), (0.0, 0.04, 2.105), p["medium_walnut"], carcass,
         bevel=0.020)
    _box("Backsplash", (3.04, 0.055, 0.29), (0.0, 0.275, 1.06), p["muted_sage"], carcass,
         bevel=0.008)

    # A hinged wall-cabinet door and sliding base drawer are true independent components.
    door_hinge = A.pivot("CabinetDoor", location=(-1.49, -0.195, 1.62), parent=root,
                         properties={"pivot_role": "left vertical hinge", "axis": "+Z"})
    door_parts = _raised_panel("AnimatedWallDoor", (-1.105, -0.205, 1.62), (0.72, 0.038, 0.70),
                               m["dark_walnut"], m["walnut_inset"], door_hinge)
    _join_meshes(door_parts, "AnimatedWallCabinetDoor", door_hinge)
    drawer_pivot = A.pivot("CabinetDrawer", location=(0.65, -0.295, 0.67), parent=root,
                           properties={"pivot_role": "drawer slide datum", "axis": "-Y"})
    _box("AnimatedBaseDrawer", (0.72, 0.055, 0.20), (0.65, -0.302, 0.67),
         m["walnut_inset"], drawer_pivot, bevel=0.016, properties={"moving_part": "drawer"})

    # Remaining fronts are static modular examples; their sockets allow repeated runtime modules.
    static_fronts: list[bpy.types.Object] = []
    for index, (x, z, w, h) in enumerate(((-0.72, 0.48, 0.72, 0.55), (0.02, 0.48, 0.66, 0.55),
                                          (1.16, 0.48, 0.82, 0.55), (-0.30, 1.62, 0.72, 0.70),
                                          (0.47, 1.62, 0.72, 0.70), (1.24, 1.62, 0.72, 0.70)), 1):
        static_fronts.extend(_raised_panel(f"StaticFront{index}", (x, -0.205 if z > 1.0 else -0.292, z),
                                           (w, 0.038, h), m["dark_walnut"], m["walnut_inset"], fronts))
    _join_meshes(static_fronts, "StaticCabinetFrontSet", fronts)
    _box("InternalShelf", (0.66, 0.39, 0.035), (-1.11, 0.045, 1.62), p["natural_oak"], carcass,
         bevel=0.008, properties={"visible_when_door_open": True})

    _join_meshes(_brass_pull("AnimatedDoorPull", (-1.10, -0.232, 1.62), 0.13,
                              door_hinge, p["restrained_brass"]),
                 "AnimatedDoorBrassPull", door_hinge)
    _join_meshes(_brass_pull("AnimatedDrawerPull", (0.65, -0.337, 0.67), 0.13,
                              drawer_pivot, p["restrained_brass"]),
                 "AnimatedDrawerBrassPull", drawer_pivot)
    pulls: list[bpy.types.Object] = []
    for i, (x, y, z) in enumerate(((-0.72, -0.322, 0.48), (0.02, -0.322, 0.48),
                                   (1.16, -0.322, 0.48), (-0.30, -0.232, 1.62),
                                   (0.47, -0.232, 1.62), (1.24, -0.232, 1.62)), 1):
    ...
    _marker("InternalShelf", root, (-1.11, 0.045, 1.65), visible_when_door_open=True)

    collision = _group("CabinetCollision", root)
    A.collision_box("BaseCabinetHull", (3.08, 0.57, 0.86), (0.0, 0.015, 0.48), parent=collision)
    A.collision_box("WallCabinetHull", (3.08, 0.48, 0.92), (0.0, 0.065, 1.61), parent=collision)
    A.collision_box("AnimatedDoor", (0.72, 0.038, 0.70), (-1.105, -0.205, 1.62),
                    parent=door_hinge, purpose="state-aware-door")
    A.collision_box("AnimatedDrawer", (0.72, 0.055, 0.20), (0.65, -0.302, 0.67),
    ...
    A.animate_transform_clip(door_hinge, "CabinetDoor_Close",
                             ...
                             interpolation="SINE")
    base = (0.65, -0.295, 0.67)
    extended = (0.65, -0.575, 0.67)
