...
async (page) => {
  ...
  const outDir = process.env.QA_OUT_DIR || path.join(
    repo, 'qa', 'checkout-delivery-groundskeeping-balance', 'current', 'maintenance',
  );
  const target = process.env.QA_BASE_URL || 'http://127.0.0.1:18457/';
