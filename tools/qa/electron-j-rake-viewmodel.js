// J (Goal 23) — THE BROOM HEAD'S ROLL, SWEPT IN ELECTRON, FOR THE OWNER TO PICK.
//
// WHAT THE OLD SWEEP MEASURED: tools/qa/rake-pitch-sweep.js boots against
...
//
//   node tools/qa/run-electron.cjs tools/qa/electron-e-rake-roll-sweep.js --clubhouse=pine-hills-v2
async (page) => {
  const fs = process.getBuiltinModule('node:fs');
  const path = process.getBuiltinModule('node:path');
  const OUT = path.resolve('qa/electron/j-rake');
  ...
  // has thirteen committed reference images proving the rake renders from it.
  ...
  out.setup = await page.evaluate(() => {
    ...
    // OUTDOORS. The rake, hose and divot kit are the OUTDOOR belt; the broom,
    // mop and vacuum are the indoor one. The golden tool pose is inside, and at
    // it the rake reported equipped:true and vmActive:true and drew NOTHING --
    // the same reading the mop gave outdoors. The theory this tests is that a
    // tool viewmodel draws only in the domain its tool belongs to.
    w.x = ip.x + 1.0; w.z = ip.z + 16.0;
    w.yaw = Math.PI; w.pitch = -0.12;
    w.vx = 0; w.vz = 0;
  ...
  out.equipped = await page.evaluate(() => {
    try { window.__fw.scene3d.walk.setTool('rake'); return true; } catch (e) { return String(e.message || e); }
  });
  await page.waitForTimeout(4000);
  out.vmActive = await page.evaluate(() => (
    window.__fw.scene3d.walk.rakeDiagnostics?.()?.vmActive
    ?? window.__fw.scene3d.walk.heldToolDiagnostics?.()?.vmActive
    ?? null
  ));

  // ONE PHOTOGRAPH, not a sweep. "The bunker rake viewmodel -- deformed lumps
  ...
  out.candidates.push({ n: 1, file, caption: 'bunker rake, golden tool pose' });
