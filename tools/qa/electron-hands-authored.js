...
async (page) => {
  ...
  out.adopt = await page.evaluate(() => ({ adopt: window.__fwHandAdopt ?? null, load: window.__fwHandLoad ?? null })); for (const k of ['fpHands','hands']) { const h = s3[k]; if (h?.authoredHandDiagnostics) return h.authoredHandDiagnostics(); } return null; })());
