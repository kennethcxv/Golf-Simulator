async (page) => {
  ...
  const outDir = path.join(repoRoot, 'qa', 'clothing_racks', 'baseline');
  ...
  const cameras = [
    ...
    { id: '02-apparel-bay', atLocal: [-4.0, 3.4], targetLocal: [-5.4, 0.0], pitch: -0.02 },
    { id: '03-retail-floor', atLocal: [2.3, 1.7], targetLocal: [-2.8, 0.0], pitch: -0.04 },
  ];
  ...
  const evaluated = await cdp.send('Runtime.evaluate', { expression: 'window', objectGroup: 'clothing-rack-listeners' });
  const listenerResult = await cdp.send('DOMDebugger.getEventListeners', { objectId: evaluated.result.objectId });
  await cdp.send('Runtime.releaseObjectGroup', { objectGroup: 'clothing-rack-listeners' });

  return {
    ok: diagnostics.consoleErrors.length === 0 && diagnostics.pageErrors.length === 0,
    phase: 'untouched-baseline',
    launch: 'node tools/qa/run-playwright.cjs tools/qa/clothing-racks-baseline.js --bootstrap',
