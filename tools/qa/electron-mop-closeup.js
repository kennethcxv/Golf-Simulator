...
async (page) => {
  ...
  const OUT = path.resolve('qa/electron/mop-closeup');
  ...
  out.acceptance = await photographTool(page, 'mop', path.join(OUT, 'acceptance-default-camera.png'));
  ...
  out.verdict = {
    ...
    allHeldTheTool: out.exhibit.every((e) => e.tool === 'mop' && e.drawable > 0),
  ...
  fs.writeFileSync(path.join(OUT, 'mop-closeup.json'), `${JSON.stringify(out, null, 2)}\n`);
  return out;
}
