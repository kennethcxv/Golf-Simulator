...
async function setupFixture(page) {
  return page.evaluate(async ({ customerName, teeMinute }) => {
    ...
    walk.x = layout.REGISTER.stand.x + off.x;
    walk.z = layout.REGISTER.stand.z + off.z;
    const dx = layout.REGISTER.monitor.x - layout.REGISTER.stand.x;
    const dz = layout.REGISTER.monitor.z - layout.REGISTER.stand.z;
