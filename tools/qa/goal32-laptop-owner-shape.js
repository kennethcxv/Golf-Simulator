...
async (page) => {
  ...
  // qa-boot's Continue check is one immediate evaluate, but the menu enables
  // Continue only after the async save inspection resolves — a big seeded
  // save loses that race and the boot silently falls to new-game (which then
  // OVERWRITES the seeded autosave). This profile is seeded on purpose:
  // wait for the enabled Continue first, and fail loudly if it never comes.
  ...
  if (how !== 'continue') throw new Error(`seeded profile did not resume: bootPath=${how}`);
