...
async function configureFixture(page) {
  return page.evaluate(async (skuIds) => {
    ...
    walk.x = REGISTER.stand.x + clubhouse.interior.position.x;
    walk.z = REGISTER.stand.z + clubhouse.interior.position.z;
...
async function scanAll(page) {
  ...
  await page.waitForFunction(() => {
    const tx = window.__fw.scene3d.clubhouse().register.getTx();
    return tx?.items.every((item) => item.scanned && item.staged && item.bagged);
  }, null, { timeout: 7000 });
  await waitForCameraStable(page);
}

async function clickPresentedCard(page) {
  // Card presentation and chip insertion are automatic in the compact flow.
  await page.waitForFunction(() => {
    const tx = window.__fw.scene3d.clubhouse().register.getTx();
...
async function clickCardConfirm(page) {
  ...
  let entered = '';
  for (const digit of String(totalCents)) {
    const point = await page.evaluate((id) => (
      window.__fw.scene3d.clubhouse().register.cardKeyScreenPoint(id)
    ), `digit:${digit}`);
    assert(point?.inView, `Card reader digit ${digit} is outside the production card camera.`);
    await page.mouse.click(point.x, point.y);
    entered += digit;
...
export async function runSimplifiedRegisterPerformance(page, options = {}) {
  ...
  // The first Escape/E round trip also dismisses a one-shot document click
  // listener installed by the first-person pointer-lock bootstrap. Consume
  // that lifecycle edge before judging renderer residency so the bounded
  // batches measure repeatable register entry/exit ownership only.
  await runReentryCycles(1);
    ...
    async () => {
      ...
      await page.waitForFunction(() => {
        ...
        return register.workspace() === 'card' && tx?.method === 'card'
          && tx.stage === 'card-entry' && tx.checkoutFlow?.state === 'CardAmountEntry';
      }, null, { timeout: 7000 });
      await waitForCameraStable(page);
    },
    { heapControl: heapIdleControl },
  ...
  dynamicPhases.cardInsertion = deriveDynamicSubphase(
    dynamicWindows.scanAndCardHandoff,
    'cardInsertion',
    'Automatic physical card presentation and chip insertion',
    (entry) => ['CardPresented', 'CardInsertReady', 'CardInserting', 'CardAmountEntry']
      .includes(entry.checkoutFlowState),
    'Frames selected from the uninterrupted scan/handoff window while the physical card moved from the customer to the chip reader.',
  );
  await page.evaluate(() => {
    // Production approvals are deterministic. A negative diagnostic sample is
    // outside normal RNG output and deliberately exercises the retained decline
    // path without adding a production-only force switch.
    window.__fw.scene3d.clubhouse().register.getTx().rng = () => -1;
  });
  await page.mouse.move(VIEWPORT.width / 2, VIEWPORT.height / 2);
  await waitForCameraStable(page);
