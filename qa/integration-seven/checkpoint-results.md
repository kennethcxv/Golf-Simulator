...
## 7. Player experience polish

- Source head: `bf072a1e1d26cce631daa19d351525b4d5acf941`.
- Strategy: cherry-pick product commit `376bbb5`, lifecycle fix `4fe0b99`, and reusable acceptance tools from `3d15f12`; reject the branch baseline and generated-evidence commits `c540a96` and `bf072a1`.
- Integrated commits: `0290b34`, `6f416fb`, and `aaec908`; integration repair: `dc98f0b`.
- Modify/delete conflicts: `player-experience-checkout-baseline.mjs` and `player-experience-operations-baseline.mjs` were kept deleted. Their prerequisites were intentionally unselected, they duplicated stronger integrated routes, and one relied on a personal-path substitution. The production stress, performance, visual-loop, settings, and save-failure tools were retained.
- Canonical UX boundary: one notification queue, one modal/dialog implementation, one tutorial/settings persistence path, one tool wheel, one audio service, and one presentation-mode arbiter remain. The integrated economy marketplace tier gate, guarded property sale, tee desk, maintenance tablet, placement catalog, delivery flow, retail register, and laptop all retain their gameplay ownership.
- Input resolution: pause owns `P` in every major mode, including the tee desk. Tool use is stopped on pause, scene/menu changes, laptop/front-desk entry, backgrounding, and page teardown. A maintenance stroke is finalized exactly once before changing tools or modes. Only explicit selection or dismissal reacquires look control; lifecycle closures do not steal pointer focus.
- Tool-wheel resolution: the indoor belt contains hands and the shop vacuum; the outdoor belt contains the ten canonical hands/washing/maintenance entries. Number `0` selects the tenth entry. A dense radial layout keeps all labels readable at the Electron minimum viewport.
- QA repair: the register stress fixture now clears organic customers through the customer API, introduces stock through `inventoryLifecycle`, waits for a real retail transaction, and asserts that the shared counter prompt prioritizes checkout over the tee desk before using normal `E` input. Tool selection is label-based rather than tied to a stale branch index.
- Portability repair: tracked legacy browser tools derive their URL and evidence root from `GOLF_FLIPPER_URL`, `GOLF_FLIPPER_QA_ROOT`, and the current working directory. The inventory-report generator discovers the excluded worktree from Git metadata rather than embedding a personal path.
- Dependencies: the completed branches changed no packages. Integration adds exact dev dependency `playwright@1.61.1` because the repository tracks Playwright QA tools but previously did not declare their runner. The lockfile was regenerated by npm; no runtime dependency was added.
- Parser: all 229 JavaScript/CommonJS/ESM files passed. Focused post-resolution matrix passed 166/166, the repair matrix passed 66/66, and the final full suite passed 628/628 in 170.1 seconds. Diff and conflict-marker checks passed.
- Browser stress: 100 pause/resume cycles, 100 camera transitions, a live retail-register pause, notification deduplication, listener balance, and background audio suspension passed with zero console errors, zero page errors, and zero listener growth.
- Visual loop: sixteen normal-control captures passed with zero console/page errors. Both the two-item indoor belt and ten-item outdoor belt were inspected; the dense outdoor ring has no overlap or clipping at 1440 x 900. Failed requests were only deliberately aborted optional GLB probes with procedural fallbacks.
- Performance checkpoint: the fixed exterior sample recorded 30.23 mean FPS, 5.85 one-percent-low FPS, 6,053 median draw calls, 117.7 MB final JS heap, and zero listener growth. This is not accepted as an improvement claim: an identical clean-main baseline and broader integrated scenario comparison remain final gates.
- Decision: accept the reconciled product UX and reusable acceptance tools. Keep branch-generated media out of product history; the final integrated visual-review set supersedes it.
