...
returns the face at its native 768×512, the way the register hands over
`cardBrandCanvas`. Looking at it immediately caught something — and one of the
somethings was mine.

The footer read *"E close the book"* and *"next page D"*. Against the handler
that is wrong twice: `interact` (E) turns the page **forward**, `dirtSense` (Q)
closes, and **D is not handled at all** — Goal 20's F3 and F4 were about exactly
those keys. **I nearly reported it.** `main.js` pushes the live bindings through
`setControlLabels` on every open, so no player has seen those strings; I saw them
because my driver opens the book directly and bypasses the push. **Seventh
instrument fault of the session.**

What *is* real: the **fallbacks** contradicted the code. A default saying D turns
the page is a trap either way — it shows if the push is ever missed, and it is
what anyone reading the file believes. They match the handler now.

**What the spread does well:** a clear masthead, a rule, a contents list with
page numbers, locks on sections not yet earned, a running footer.
**What F1 asks for and it still does not do:** say *where you are* (the footer
prints "2", not "2 of 9", and a content page has no breadcrumb to its section),
and offer any way to reach a section from the contents other than turning pages
one at a time. **The rebuild is NOT DONE — and it can now be argued about from
pictures instead of from memory.**
