...
14. **Kill QA process TREES, never shells.** A `TaskStop`/shell kill on Windows leaves the runner and its browser running; the orphan keeps taking the run lock and silently contaminates every timing-sensitive run that follows. `taskkill /PID <pid> /T /F` on the chain AND the runner, then verify zero surviving `run-playwright` processes. (Learned 2026-07-28: a "stopped" chain ran nine more steps including an 83-minute measurement.)
15. **One QA chain at a time, verified — the lock is not chain-aware.** The run lock serializes individual runner invocations at best, and QA-LOCK-001 records a window where even that failed. Before launching any wall-clock-sensitive run, verify no other QA process exists; never rely on the lock to protect a measurement from a chain you believe you stopped.
