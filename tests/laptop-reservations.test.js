...
import {
  bookLaptopReservation, laptopPartySizeLimit, laptopReservationSheet,
} from '../src/ui/laptop.js';

const tomorrow = (state) => calendarOf(state.clock.minutes).dayAbs + 1;

test('laptop party selector reads the canonical maxPartySize configuration', () => {
  const state = newGame('relaxed', 8100);
  configureReservations(state, { maxPartySize: 4 });
  assert.equal(laptopPartySizeLimit(state), 4);

  state.reservations.config.maxPartySize = undefined;
  state.reservations.config.maxGroupSize = 3;
  assert.equal(laptopPartySizeLimit(state), 3, 'legacy maxGroupSize saves remain compatible');
});
