  ...
  try {
    ...
    for (let loadIndex = 0; loadIndex < expectedLoads.length; loadIndex += 1) {
      ...
      for (const boxId of expectedBoxIds) {
        ...
        const authority = authorityBefore.find((entry) => entry.id === boxId);
        const readyState = authority?.loc === 'pad' ? 'pallet-ready' : 'world-ready';
        assert.equal(root?.parent?.name, 'DeliveryBoxWorldRoot');
        assert.equal(root?.userData.deliveryPresentationState, readyState);
