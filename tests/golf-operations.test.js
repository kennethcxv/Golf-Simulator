...
import {
  SAVE_VERSION, dailyTick, newGame, serialize, deserialize,
...
test('midnight dailyTick owns the production booking-horizon roll', () => {
  const state = newGame('relaxed', 90511);
  const firstDay = today(state) + 1;
  setTime(state, firstDay, 0);

  assert.deepEqual(state.reservations.generator.generatedDays, []);
  dailyTick(state);
  assert.deepEqual(
    state.reservations.generator.generatedDays,
    Array.from({ length: state.reservations.config.horizonDays }, (_, offset) => firstDay + offset),
    'the first midnight exposes the configured forward horizon',
  );

  const farEdge = firstDay + state.reservations.config.horizonDays;
  setTime(state, firstDay + 1, 0);
  dailyTick(state);
  assert.ok(state.reservations.generator.generatedDays.includes(farEdge),
    'the next midnight generates the newly visible far-edge day');
});
