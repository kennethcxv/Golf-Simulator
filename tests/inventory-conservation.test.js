...
test('a unit that exists is never duplicated or lost, over 750 random actions', () => {
  ...
  for (let step = 0; step < 750; step++) {
    const shelfBeforeStep = RETAIL.reduce((sum, id) => sum + st.shop.inventory[id].shelf, 0);
    ...
    if (act === 0) {
      ...
      // Choose among stock a shopper can actually see. The production catalog
      // has grown substantially, so a uniform draw across every SKU otherwise
      // spends most of this stress run attempting empty lines and stops
      // exercising the checkout leg of the conservation invariant.
      const stocked = RETAIL.filter((id) => st.shop.inventory[id]?.shelf > 0);
      const shopperSkuId = stocked.length ? pick(stocked) : skuId;
      if (pickFromShelf(st, shopperSkuId).ok) {
        inHand[shopperSkuId] += 1;
        const sku = skuById(shopperSkuId);
        if (rand() < 0.65) {
          checkoutSale(st, [{ skuId: shopperSkuId, price: sku.price || 10 }], 'A customer');
          inHand[shopperSkuId] -= 1;
          sold[shopperSkuId] += 1;
        } else {
          returnToShelf(st, shopperSkuId);
          inHand[shopperSkuId] -= 1;
        }
      }
    } else {
    ...
    if (shelfAfterStep > shelfBeforeStep) shelfIncreaseEvents++;
  }
  ...
  assert.ok(shelfIncreaseEvents > 0, `and moved stock back onto shelves (${shelfIncreaseEvents})`);
});
...
test('a box carried and set down keeps its identity and its contents', () => {
  ...
  // The front desk now owns the old x=0, z=2 test point. This test protects
  // box identity, so select a genuinely clear floor pose through the same
  // authoritative placement preview used by gameplay instead of assuming a
  // furnishing-free coordinate forever.
  const clearFloorTargets = [
    { kind: 'surface', surfaceId: FLOOR_BOX_SURFACE_ID, x: 5, z: -6, ry: 1.2 },
    { kind: 'surface', surfaceId: FLOOR_BOX_SURFACE_ID, x: 5, z: -5, ry: 1.2 },
    { kind: 'surface', surfaceId: FLOOR_BOX_SURFACE_ID, x: 4, z: -6, ry: 1.2 },
  ];
  const target = clearFloorTargets.find((candidate) => (
    previewBoxPlacement(st, id, candidate).ok
  ));
  assert.ok(target, 'the test fixture has a clear clubhouse floor pose');
  const placed = putDownBox(st, id, target);
  ...
  assert.equal(reloaded.x, target.x);
  assert.equal(reloaded.z, target.z);
