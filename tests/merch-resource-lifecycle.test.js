...
async function withLoaderStub(load, run) {
...
test('visible-only bake excludes hidden and collision geometry and releases only owned output', async () => {
  await withLoaderStub((_url, onLoad) => onLoad({ scene: new THREE.Group(), animations: [] }), async () => {
...
test('createMerch disposal is idempotent and leaves caller material ownership intact', async () => {
  const disposed = { geometries: 0, materials: 0, textures: 0, tint: 0, base: 0 };
  let loadCount = 0;
  await withLoaderStub((url, onLoad) => {
    loadCount += 1;
...
test('late loader completions after disposal cannot resurrect prototypes or callbacks', async () => {
  const pending = [];
  await withLoaderStub((url, onLoad) => { pending.push({ url, onLoad }); }, async () => {
    const merch = createMerch({});
