...
test('a returned historical UID cannot replay its persisted lot movement', () => {
  const st = newGame('relaxed', 42);
  st.shop.inventory.glove1.shelf = 3;
  assert.equal(pickFromShelf(st, 'glove1', 'renderer-u1').ok, true);
  assert.equal(returnToShelf(st, 'glove1', 'renderer-u1').ok, true);

  const shelfBeforeReplay = st.shop.inventory.glove1.shelf;
  const replay = pickFromShelf(st, 'glove1', 'renderer-u1');
  assert.equal(replay.ok, false);
  assert.match(replay.reason, /already used/i);
  assert.equal(st.shop.inventory.glove1.shelf, shelfBeforeReplay);
  assert.deepEqual(st.shop.held, []);
});

test('generated held UIDs remain monotonic across a serialized reload', () => {
  const st = newGame('relaxed', 42);
  st.shop.inventory.glove1.shelf = 3;
  const first = pickFromShelf(st, 'glove1');
  assert.equal(first.ok, true);
  assert.equal(returnToShelf(st, 'glove1', first.uid).ok, true);

  const reloaded = JSON.parse(JSON.stringify(st));
  const second = pickFromShelf(reloaded, 'glove1');
  assert.equal(second.ok, true);
  assert.notEqual(second.uid, first.uid);
  assert.equal(reloaded.shop.inventory.glove1.shelf, 2);
  assert.deepEqual(reloaded.shop.held, [{ uid: second.uid, skuId: 'glove1' }]);
});
