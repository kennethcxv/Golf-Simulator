...
test('production contract exposes the complete physical checkout inputs', () => {
  ...
  assert.deepEqual(CHECKOUT_STATES.ProductHeld.allowedInput,
    ['cancel-product-animation']);
  assert.deepEqual(CHECKOUT_STATES.AllProductsScanned.allowedInput, ['exit-cashier']);
  assert.deepEqual(CHECKOUT_STATES.CardInsertReady.allowedInput,
    ['cancel-card-at-reader']);
  assert.deepEqual(CHECKOUT_STATES.CardInserting.allowedInput,
    ['cancel-card-at-reader']);
  assert.deepEqual(CHECKOUT_STATES.CardAmountEntry.allowedInput,
    ['key-card-digit', 'backspace-card-amount', 'clear-card-amount',
      'submit-card-amount', 'cancel-card-at-reader']);
  assert.deepEqual(CHECKOUT_STATES.CashPresented.allowedInput, ['click-presented-cash', 'exit-cashier']);
  assert.deepEqual(CHECKOUT_STATES.CashAccepted.allowedInput, ['exit-cashier']);
  assert.deepEqual(CHECKOUT_STATES.DepositingCash.allowedInput, ['exit-cashier']);
  ...
  const obsolete = new Set([
    'grab-product', 'start-card-swipe', 'move-card-swipe', 'release-card-swipe',
    'activate-card-terminal', 'open-cash-drawer', 'grab-tender-piece',
  ]);
...
test('decline and interrupted-insertion paths return to safe retry states', () => {
  assert.equal(canTransitionCheckout('CardInsertReady', 'CardInserting'), true);
...
test('major physical states cannot be skipped by one transition', () => {
  const forbidden = [
    ...
    ['CardPresented', 'CardInserting'],
    ['CardInsertReady', 'CardAmountEntry'],
...
test('automatic state timeouts recover while deliberate player waits remain untimed', () => {
  ...
  assert.equal(checkoutStateTimedOut(flow, 4_999), false);
  assert.equal(checkoutStateTimedOut(flow, 5_000), true);

  const timedOut = recoverTimedOutCheckout(flow, { nowMs: 5_000 });
  assert.equal(timedOut.ok, true);
  assert.equal(timedOut.flow.state, 'Recovery');
  assert.equal(timedOut.flow.recovery.resumeState, 'CardPresented');
