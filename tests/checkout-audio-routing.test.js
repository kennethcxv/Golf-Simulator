...
test('every active simplified checkout cue is routed from normal-play production code', () => {
  ...
  assert.equal(cueCalls(registerSource, 'cardSwipe').length, 0,
    'the compact chip-reader flow cannot route the retired swipe cue');
});
...
test('automatic card insertion and processing cues have separate one-shot edges', () => {
  const beginInsert = extractFunction(registerSource, 'beginAutomaticCardInsert');
  const handleTerminalKey = extractFunction(registerSource, 'handleTerminalKey');
  ...
  assert.equal(cueCalls(beginInsert, 'cardInsert').length, 1,
    'cardInsert fires once when automatic insertion begins');
  const transition = /flowTo\(\s*['"]CardInserting['"]/.exec(beginInsert);
  const insertCue = cueCalls(beginInsert, 'cardInsert')[0];
  assert.ok(transition && insertCue && insertCue.start > transition.index,
    'the insertion cue follows the CardInserting transition edge');
  assert.equal(cueCalls(updateCard, 'cardProcessing').length, 0,
    'the authorization timer cannot replay the processing cue');
  assert.equal(cueCalls(handleTerminalKey, 'cardProcessing').length, 1,
    'processing begins once only after exact-total confirmation');
  assert.equal(cueCalls(updateCard, 'cardApproved').length, 1);
  assert.equal(cueCalls(updateCard, 'cardDeclined').length, 1);
  assert.doesNotMatch(registerSource, /function\s+(?:startSwipe|feedSwipe|endSwipe)\s*\(|swipeAt\s*:/,
    'the active renderer exposes no competing swipe interaction surface');
});
...
test('bagging and automatic receipt cues remain transition-local one-shots', () => {
  ...
  const updateBagDropMotions = extractFunction(registerSource, 'updateBagDropMotions');
  ...
  // One click sends the item directly to the bag. The success beep and POS add belong to the
  // validated barcode-contact edge, never pickup or an unverified flight frame.
  assert.equal(cueCalls(registerSource, 'scannerActivate').length, 0,
    'the retired scanner interaction cannot play an activation cue');
  assert.equal(cueCalls(bagProduct, 'scannerActivate').length, 0);
