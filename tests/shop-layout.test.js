...
import {
  SHELL, INTERIOR, MODERN_PUBLIC_INTERIOR, FIXTURES, COUNTER, OFFICE, STOCKROOM, DOOR_MAIN,
  DOOR_CLEARWAY, BACKDOOR_CLEARWAY, CLUTTER_SPOTS, FRONT_DESK_COLLIDERS,
  TRAFFIC_PATHS, queueSlot, fixtureRect, fixtureCollisionRects, FIXTURE_HALF,
...
test('the floor plan and the sim grime grid describe the same room', () => {
  ...
  assert.ok(cw >= 1.25 && cw <= 2.5, `grime cells stay vacuum-pass sized (${cw.toFixed(2)} yd wide)`);
  assert.ok(cd >= 1.25 && cd <= 2.5, `grime cells stay vacuum-pass sized (${cd.toFixed(2)} yd deep)`);
...
test('the starter clubhouse stays near 1,900 square feet instead of the oversized hall', () => {
  const footprintSqFt = SHELL.w * SHELL.d * 9;
  assert.ok(Math.abs(footprintSqFt - 1904.4) < 1e-9);
  assert.ok(footprintSqFt <= 1950, `${footprintSqFt} sq ft remains a compact starter property`);
});
...
test('physical fixtures and randomized starter clutter clear every modern service-room doorway', () => {
  const s = 1 / 0.9144;
  const routes = [
    // The opening is just over 1 yd wide around this player-centre lane. Its
    // remaining width is reserved for the 0.34 yd player capsule itself.
    { name: 'employee', minX: 4.7, maxX: 7.8, minZ: 3.70 * s - 0.16, maxZ: 3.70 * s + 0.16 },
    { name: 'storage', minX: 4.7, maxX: 7.8, minZ: -0.20 * s - 0.16, maxZ: -0.20 * s + 0.16 },
    { name: 'restroom', minX: 4.7, maxX: 7.8, minZ: -4.18 * s - 0.16, maxZ: -4.18 * s + 0.16 },
  ];
  const overlaps = (a, b) => a.maxX > b.minX && a.minX < b.maxX
    && a.maxZ > b.minZ && a.minZ < b.maxZ;
  for (const fixture of FIXTURES) {
    for (const rect of fixtureCollisionRects(fixture)) {
      for (const route of routes) {
        assert.equal(overlaps(rect, route), false, `${fixture.id} clears the ${route.name} doorway`);
      }
    }
  }
  for (const seed of [1, 42, 31415, 424242]) {
    for (const [index, pile] of newGame('relaxed', seed).shop.reno.clutter.entries()) {
      const rect = {
        minX: pile.x - 0.45, maxX: pile.x + 0.45,
        minZ: pile.z - 0.45, maxZ: pile.z + 0.45,
      };
      for (const route of routes) {
        assert.equal(overlaps(rect, route), false,
          `seed ${seed} clutter ${index} clears the ${route.name} doorway`);
      }
    }
  }
});

const rectsOverlap = (a, b) => a.minX < b.maxX && a.maxX > b.minX
  && a.minZ < b.maxZ && a.maxZ > b.minZ;

test('fresh and migrated clutter clears the complete Pine Hills desk footprint', () => {
  const assertDeskClear = (piles, label) => {
    for (const [index, pile] of piles.entries()) {
      ...
      for (const [run, desk] of Object.entries(FRONT_DESK_COLLIDERS)) {
        assert.equal(rectsOverlap(footprint, desk), false,
          `${label} clutter ${index} clears the ${run}`);
      }
    }
  };

  const fresh = newGame('relaxed', 31415);
  assertDeskClear(fresh.shop.reno.clutter, 'fresh');
  const raw = JSON.parse(serialize(fresh));
  raw.shop.reno.layoutVersion = 2;
  raw.shop.reno.clutterLayout = 2;
  raw.shop.reno.clutter[2] = { x: 1.4, z: 0.5, ry: 0.2, cleared: true };
  const migrated = deserialize(JSON.stringify(raw));
  assertDeskClear(migrated.shop.reno.clutter, 'migrated');
  assert.equal(migrated.shop.reno.clutter[2].cleared, true,
    'desk-safe relocation preserves completed hauling progress');
  assert.equal(migrated.shop.reno.clutterLayout, RENO.clutterLayout);
});

test('the rendered grime trail follows the canonical front-desk queue', () => {
  assert.deepEqual(TRAFFIC_PATHS[1].slice(-2), [queueSlot(1), queueSlot(0)]);
});
  ...
  for (const [id, n] of Object.entries(counts)) {
    ...
    for (const s of DECOR_SPOTS[id]) {
      ...
      if (s.mount === 'wall') {
        ...
        const flushModernX = Math.abs(Math.abs(s.x) - MODERN_PUBLIC_INTERIOR.w / 2) < 0.15;
        const flushModernZ = Math.abs(Math.abs(s.z) - MODERN_PUBLIC_INTERIOR.d / 2) < 0.15;
        const flushPartition = Math.abs(s.x - STOCKROOM.bounds.minX) < 0.15;
        assert.ok(flushX || flushZ || flushModernX || flushModernZ || flushPartition,
          `${id} wall spot hangs on a real wall`);
      }
    }
  }
});
