...
test('first-person hands resample live authored grip positions without changing pose', () => {
  ...
  try {
    ...
    assert.deepEqual(hands.root.getObjectByName('Hand_Right').position.toArray(), [0.2, -0.1, 0.5]);
    assert.deepEqual(hands.root.getObjectByName('Hand_Left').position.toArray(), [-0.2, -0.3, -0.6]);
  } finally {
    hands.dispose();
  }
});
