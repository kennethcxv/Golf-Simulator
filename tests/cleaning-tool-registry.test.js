...
test('the authored spray trigger ships attached at its hinge, needing no runtime fix', async () => {
  const built = buildToolViewmodels();
  // The builder now re-parents PIVOT_Trigger onto the identity asset root before export. The
  // held LOD group's own transform otherwise makes the glTF exporter mislocate the animated
  // empty (the blade ended up on the floor), so the earlier fix was a runtime hack that
  // restored the pivot to SOCKET_Trigger. With the export corrected, the GLB ships the pivot
  // at the hinge with an identity rest rotation and the viewmodel adopts it untouched — no hack.
  const HINGE = [0, -0.03, 0.079];
  const loader = {
    load(url, onLoad) {
      const scene = new THREE.Group();
      const held = new THREE.Group();
      held.name = 'LOD0_SprayHeld';
      scene.add(held);
      const animations = [];
      if (url.includes('asset_076_')) {
        ...
        pivot.position.set(...HINGE); // shipped at the hinge, under the root, as authored now
        scene.add(pivot);
        const blade = new THREE.Mesh(
          new THREE.BoxGeometry(0.02, 0.01, 0.04), new THREE.MeshStandardMaterial(),
        );
        blade.name = 'MESH_SprayTriggerBlade';
        blade.position.set(0, 0.006, -0.02);
        pivot.add(blade);
  ...
  try {
    ...
    const pivot = built.groups.spray.getObjectByName('PIVOT_Trigger');
    const blade = built.groups.spray.getObjectByName('MESH_SprayTriggerBlade');
    assert.ok(pivot && blade, 'the authored trigger pivot and blade adopt');
    // The runtime must leave the pivot exactly where the GLB placed it — no compensation.
    assert.deepEqual(pivot.position.toArray(), HINGE,
      'the adopted pivot keeps its authored hinge position (no restoration hack)');
    assert.equal(blade.parent, pivot, 'the blade stays attached to the pivot');
