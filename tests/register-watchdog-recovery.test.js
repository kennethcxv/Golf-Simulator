...
test('live delivery holds BagHandoff and CustomerLeaving around their physical windows', () => {
  ...
  const beginBagStart = source.indexOf('  function beginBagDeliveryOrRelease() {');
  const beginBagEnd = source.indexOf('\n  function holdBagAtCustomer() {', beginBagStart);
  assert.ok(beginBagStart >= 0 && beginBagEnd > beginBagStart);
  const beginBag = source.slice(beginBagStart, beginBagEnd);

  const bagHandoff = beginBag.indexOf("'BagHandoff'");
  const bagMotion = beginBag.indexOf("deliveryPhase = 'bag-deliver'");
  const customerLeaving = delivery.indexOf("flowTo('CustomerLeaving'");
