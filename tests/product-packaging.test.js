...
test('SKU quantities equal exact socket capacities rather than a representative visual count', () => {
  const expectedRetailCapacity = {
    ...
    cap1: 8, cap2: 8, visor1: 8, jacket2: 8, shoe1: 4, shoe3: 4, sock1: 8,
    tees1: 12, towel1: 12, marker1: 12, divot1: 12, range2: 4,
    sunglasses2: 12, bottle1: 12, scorecard1: 8,
    sportdrink2: 12, soda1: 12, chips1: 12, bar2: 12, crackers1: 12,
    umb1: 6, bag1: 1, bag3: 1,
    water1: 12, snack1: 12,
  };
  const retailIds = SHOP_CATALOG.filter((sku) => RETAIL_CATS.has(sku.cat)).map((sku) => sku.id).sort();
  assert.deepEqual(Object.keys(expectedRetailCapacity).sort(), retailIds,
    'every sellable SKU retains an audited carton quantity');

  for (const skuId of PRODUCT_PACKAGING_SKU_IDS) {
    const contract = productPackagingFor(skuId);
    const layout = PACKAGING_LAYOUTS[contract.layoutId];
    const capacity = contract.retail ? expectedRetailCapacity[skuId] : 1;
