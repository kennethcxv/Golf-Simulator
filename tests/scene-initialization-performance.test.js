...
test('loading a scene does not generate paid tee-time demand as a startup side effect', () => {
  const start = mainSource.indexOf('function startGameNow(');
  const end = mainSource.indexOf('\n// full-screen loading veil', start);
  assert.ok(start >= 0 && end > start, 'startGameNow lifecycle must exist');
  const body = mainSource.slice(start, end);

  assert.doesNotMatch(body, /ensureReservationHorizon\(/,
    'Continue and manual Load must preserve saved cash and reservation ledgers');
  assert.match(body, /Loading a club is a pure restore boundary/);
});
