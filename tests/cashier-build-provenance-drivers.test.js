...
test('success and blocker paths retain every driver-specific screenshot reference', () => {
  ...
  const finalScanRelease = performance.indexOf('item.scanned && item.bagged', scanReleaseWait);
  ...
  const persistenceFinalRelease = saveReload.indexOf('item.scanned && item.bagged', persistenceScanRelease);
