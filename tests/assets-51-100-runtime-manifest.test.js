...
test('small authored practicals use their emissive meshes instead of global fragment lights', () => {
  for (const number of [83, 94, 95]) {
    const light = RUNTIME_ASSET_MANIFEST_BY_NUMBER[number].placement.light;
    assert.equal(light?.presentation, 'emissive-only',
      `asset ${number} must preserve its glow without entering the physical light loop`);
  }
});
