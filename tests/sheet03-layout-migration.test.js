...
test('apparel table render and placement share the Pine Hills safety envelope', () => {
...
test('pre-current saves restore only invalid moved poses under repaired footprints', () => {
  ...
  // This remains legal both on the original basic floor and after a pre-v12
  // save receives its compatibility-only full fixture set.
  const validTablePose = { x: -5.75, z: 1.5, ry: 0 };
  assert.equal(
    validatePlacement(
      state,
      'table_polos',
  ...
  const restoredShoerack = placedFixtures(migrated).find((fixture) => fixture.id === 'shoerack');
  const authoredShoerack = fixtureById('shoerack');
  assert.deepEqual(
    {
      id: restoredShoerack.id,
      x: restoredShoerack.x,
      z: restoredShoerack.z,
      ry: restoredShoerack.ry,
    },
    {
      id: authoredShoerack.id,
      x: authoredShoerack.x,
      z: authoredShoerack.z,
      ry: authoredShoerack.ry,
    },
