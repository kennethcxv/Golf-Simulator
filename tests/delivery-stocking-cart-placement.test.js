...
test('picking a cart box clears ownership and it can be replaced exactly once', () => {
  ...
  assert.ok(pickUpBox(st, box.id).ok);
  assert.ok(putDownBox(st, box.id, floorTarget(6.3, -2.8, 0.4)).ok);
...
test('legacy aliases migrate and malformed, duplicate or incompatible placements heal to stock', () => {
  ...
  st.shop.deliveries = {
    ...
    boxes: [
      ...
      { id: 6, skuId: 'tees1', orderId: 6, qty: 5, cap: 12, box: 'carton', loc: 'world', surfaceId: FLOOR_BOX_SURFACE_ID, x: 6.3, z: -2.8, ry: 0, equipmentId: 'stocking_cart', socketId: 'STOCK_SOCKET_06' },
  ...
  assert.deepEqual({ x: healed[5].x, z: healed[5].z }, { x: 6.3, z: -2.8 });
