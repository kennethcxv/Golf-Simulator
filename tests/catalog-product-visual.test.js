...
import {
  FRONT_DESK_FRAME, REGISTER, frontDeskLocalPoint,
...
async function loadProductScenes() {
  ...
  const models = [...new Set(retail
    ...
    .filter((visual) => !visual.raw && visual.model)
...
test('the entire catalog resolves explicit physical descriptors aligned to exact delivery contracts', () => {
  ...
  for (const sku of SHOP_CATALOG) {
    ...
    const exactPackedProxy = !packaging.retail
      && visual.raw
      && visual.kind.startsWith('packed-');
    assert.ok(String(visual.model).length > 5 || exactPackedProxy,
...
test('all catalog SKU selections satisfy their shipped carton layout metadata at runtime', async () => {
...
test('size-aware layout keeps oversize products flat and separated from compact goods', () => {
  ...
  const localPoses = poses.map((pose) => frontDeskLocalPoint(pose.x, pose.z));
  const localCorners = [
  ...
  const centerX = (
    Math.min(...localCorners.map((point) => point.x))
    + Math.max(...localCorners.map((point) => point.x))
  ) / 2;
  assert.deepEqual(poses.map((pose) => pose.sizeClass), ['oversize', 'compact', 'oversize']);
  assert.ok(Math.abs(localPoses[0].x - centerX) < 1e-9);
  assert.ok(Math.abs(localPoses[2].x - localPoses[0].x) < 1e-9);
  assert.ok(localPoses[1].z > localPoses[0].z,
    'compact item uses the clear local row behind long goods');
  const localYaw = ((poses[0].ry - FRONT_DESK_FRAME.ry + Math.PI) % (Math.PI * 2)
    + Math.PI * 2) % (Math.PI * 2) - Math.PI;
  assert.ok(Math.abs(localYaw + 0.05) < 1e-9, 'product yaw rotates with the desk frame');
});
...
test('shipped family GLBs retain authored anchors, collisions, dimensions and tint slots', async () => {
  ...
  for (const [model, scene] of scenes) {
    ...
    const orderedAxes = size.toArray().sort((a, b) => a - b);
    assert.ok(orderedAxes[0] > 0.001 && orderedAxes[1] > 0.02 && orderedAxes[2] > 0.02,
      `${model} has non-degenerate authored bounds, including legitimate thin cards`);
...
test('sibling SKU tiers use distinct identity tints backed by one cached material per tint', async () => {
  ...
  try {
    ...
    merch = createMerch({ merchPlastic: accentBase, merchFabric: fabricBase, charcoal: fabricBase });
    await new Promise((resolve) => merch.onReady(resolve));
