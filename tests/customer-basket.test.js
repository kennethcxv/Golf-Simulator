...
test('production clubhouse binds every customer-basket presentation transition it calls', () => {
  const source = fs.readFileSync(
    new URL('../src/render3d/clubhouse.js', import.meta.url),
    'utf8',
  );
  const imported = source.match(
    /import\s*\{([^}]+)\}\s*from '\.\.\/sim\/customerBasket\.js';/,
  )?.[1] || '';
  for (const helper of ['abandonUnit', 'stageUnit', 'visibleBasketSlots']) {
    assert.match(imported, new RegExp(`\\b${helper}\\b`), `${helper} is imported by the live renderer`);
  }
});
