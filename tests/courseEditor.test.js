...
test('objects: place, refuse greens, move, duplicate, remove, scatter, undo chain', () => {
  ...
  const moveTarget = [
    ...
    [QX + 3, QY + 3], [QX - 3, QY - 3],
  ].find(([x, y]) => objectPlacementOk(st.course, 'tree_oak', x, y, {
    ignoreId: a.object.id,
  }).ok);
  assert.ok(moveTarget, 'the quiet scrub has a legal nearby move target');
  const mv = moveObject(st, s, a.object.id, { x: moveTarget[0], y: moveTarget[1], rot: 1.2 });
