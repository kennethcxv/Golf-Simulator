...
import {
  FRONT_DESK, LOUNGE, MODERN_PUBLIC_RESTROOM, OFFICE, STOCKROOM,
} from '../src/data/shopLayout.js';
import { DECOR_SPOTS } from '../src/data/shopItems.js';
import {
  BOX_PLACEMENT_CODES,
...
test('floor previews reject every audited non-fixture solid, including STOCKROOM.bin', () => {
  ...
  for (const probe of [
    ...
    { id: 'front-desk-chair', x: FRONT_DESK.staffChair.x, z: FRONT_DESK.staffChair.z },
    { id: 'office-filing-cabinet', x: OFFICE.filing.x, z: OFFICE.filing.z },
    { id: 'cleaning-corner', x: STOCKROOM.cleaning.x, z: STOCKROOM.cleaning.z },
    { id: 'lounge-chair-a', x: LOUNGE.chairA.x, z: LOUNGE.chairA.z },
    // Use the table's north edge, clear of both adjacent chairs.
    { id: 'lounge-coffee-table', x: LOUNGE.coffee.x, z: -3.70 },
    { id: 'retail-gondola', x: 0.40, z: -0.90 },
    { id: 'restroom-toilet', x: MODERN_PUBLIC_RESTROOM.toilet.x, z: MODERN_PUBLIC_RESTROOM.toilet.z },
    { id: 'restroom-sink', x: MODERN_PUBLIC_RESTROOM.sink.x, z: MODERN_PUBLIC_RESTROOM.sink.z },
    // Probe north of the separately reserved receiving-door clearway so this
    // assertion identifies the permanent wall itself.
    { id: 'restroom-east-wall', x: MODERN_PUBLIC_RESTROOM.eastWallX, z: -5.15 },
  ]) {
...
test('legacy world cartons inside the new restroom enclosure heal to stock without data loss', () => {
  const state = newGame('relaxed', 1211);
  state.shop.reno.clutter.forEach((pile) => { pile.cleared = true; });
  const box = currentBox(91, {
    loc: 'world',
    surfaceId: FLOOR_BOX_SURFACE_ID,
    x: MODERN_PUBLIC_RESTROOM.toilet.x,
    z: MODERN_PUBLIC_RESTROOM.toilet.z,
    ry: 0.2,
    qty: 7,
  });
  state.shop.deliveries.boxes = [box];
  ensureDeliveries(state);
  assert.equal(box.loc, 'stock');
  assert.equal(box.qty, 7);
  assert.equal(box.lifecycle, BOX_LIFECYCLE.SEALED);
...
test('live clutter and placed floor decor participate in the same floor authority', () => {
  ...
  const box = currentBox(1, { loc: 'carried' });
  state.shop.reno.clutter = [{ x: -5, z: 1, ry: 0.4, cleared: false }];
  state.shop.reno.decor = [];

  const clutter = previewBoxPlacement(
    state,
    box,
    surfaceTarget(FLOOR_BOX_SURFACE_ID, -5, 1),
  );
  ...
  const plantPose = DECOR_SPOTS.plant1[1];
  const plant = previewBoxPlacement(
    state,
    box,
    surfaceTarget(FLOOR_BOX_SURFACE_ID, plantPose.x, plantPose.z),
  );
...
test('fresh current-schema loads heal fully in stable save order without duplication', () => {
  ...
  const boxes = [
    ...
    currentBox(4, { loc: 'world', surfaceId: FLOOR_BOX_SURFACE_ID, x: -5, z: 1, ry: 0 }),
    currentBox(5, { loc: 'world', surfaceId: FLOOR_BOX_SURFACE_ID, x: -4.9, z: 1, ry: 0 }),
    currentBox(6, {
...
test('dynamic-blocker changes invalidate cached healing without touching legal cartons', () => {
  ...
  const box = currentBox(1, {
    loc: 'world', surfaceId: FLOOR_BOX_SURFACE_ID, x: -5, z: 1, ry: 0,
  });
  state.shop.deliveries = {
    schemaVersion: DELIVERIES_SCHEMA_VERSION,
    boxes: [box],
  ...
  state.shop.reno.clutter = [{ x: -5, z: 1, ry: 0, cleared: false }];
  ensureDeliveries(state);
  assert.equal(box.loc, 'stock');
