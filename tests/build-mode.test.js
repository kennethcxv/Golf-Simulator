...
test('a moved fixture is rejected when its authored local-front browse target leaves the shop', () => {
  const st = fresh();
  st.shop.progression.tier = 'premium';
  const feature = FIXTURES.find((fixture) => fixture.id === 'table_polos');
  const candidate = { ...feature, x: 6, z: 4.25, ry: 0 };
...
test('intermediate snack_rack layout saves migrate once to the stable snackrack ID', () => {
  const st = fresh();
  const legacyRecord = {
    assetId: 'generated-fixture:snack_rack',
    state: 'stored',
    transform: { x: -5.5, y: 0, z: 5.5, ry: Math.PI, surface: 'floor' },
    variant: 'original',
    requiredRelationship: null,
  };
  st.shop.layout.moved.snack_rack = { x: -5.5, z: 5.5, ry: Math.PI };
  st.shop.layout.stored.push('snack_rack');
  st.shop.layout.objects.snack_rack = legacyRecord;

  ensureLayout(st);
  assert.deepEqual(st.shop.layout.moved.snackrack, { x: -5.5, z: 5.5, ry: Math.PI });
  assert.deepEqual(st.shop.layout.stored, ['snackrack']);
  assert.deepEqual(st.shop.layout.objects.snackrack, {
    ...legacyRecord,
    assetId: 'generated-fixture:snackrack',
  });
  assert.equal('snack_rack' in st.shop.layout.moved, false);
  assert.equal('snack_rack' in st.shop.layout.objects, false);

  const once = JSON.stringify(st.shop.layout);
  ensureLayout(st);
  assert.equal(JSON.stringify(st.shop.layout), once, 'the alias bridge is idempotent');

  const extra = fresh();
  extra.shop.layout.extra.push({
    id: 'snack_rack', kind: 'snackrack', x: -5.25, z: 5.75, ry: Math.PI,
  });
  ensureLayout(extra);
  assert.equal(extra.shop.layout.extra.some((fixture) => fixture.id === 'snack_rack'), false);
  assert.equal(fixtureById(extra, 'snackrack').x, -5.25);
  assert.equal(fixtureById(extra, 'snackrack').z, 5.75);
});
