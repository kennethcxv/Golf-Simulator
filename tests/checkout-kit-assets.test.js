...
test('the apparel table top carries the polo1 lane on two authored stack positions', async () => {
  const kit = await kitPromise;
  ...
  assert.equal(poses.length, 6, 'four apparel lines divide the table into two three-high stacks each');
...
test('all four apparel-table lines use distinct authored stack columns', async () => {
  const kit = await kitPromise;
  const scene = (await kit.get('apparel_table')).scene;
  scene.updateMatrixWorld(true);
  const sockets = [];
  scene.traverse((object) => {
    if (object.name.startsWith('APPAREL_TABLE_SLOT_')) {
      sockets.push(object.getWorldPosition(new THREE.Vector3()));
    }
  });
  const laneIds = ['polo1', 'polo2', 'pants2', 'shorts1'];
  const laneXs = [];
  for (const skuId of laneIds) {
    const poses = slotsFor(skuId);
    assert.equal(poses.length, 6, `${skuId} owns two three-high stacks`);
    assert.ok(poses.every((pose) => pose.folded));
    laneXs.push(poses[0].x);
    for (const pose of poses) {
      assert.ok(sockets.some((socket) => (
        Math.abs(socket.x - pose.x) < 0.03 && Math.abs(socket.z - pose.z) < 0.03
      )), `${skuId} lands on an authored table socket`);
    }
  }
  assert.equal(new Set(laneXs).size, laneIds.length, 'each line owns a distinct table column');
});
