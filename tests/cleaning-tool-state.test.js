...
test('the final viable wring reaches one canonical empty bucket state', () => {
  const state = newGame('relaxed', 951);
  let last = null;
  for (let wring = 0; wring < 18; wring += 1) {
    last = serviceMop(state);
    assert.equal(last.ok, true, `wring ${wring + 1} remains usable`);
  }

  const c = ensureCleaningToolState(state);
  assert.equal(last.water, 'empty');
  assert.equal(last.level, 0);
  assert.equal(c.bucket.level, 0);
  assert.equal(c.bucket.water, 'empty');
  assert.equal(c.bucket.wrings, 18);
  assert.equal(serviceMop(state).reason, 'bucket-empty');
  assert.equal(c.bucket.wrings, 18, 'a rejected extra press cannot create another wring');

  const persisted = ensureCleaningToolState(reload(state));
  assert.deepEqual(
    { level: persisted.bucket.level, water: persisted.bucket.water, wrings: persisted.bucket.wrings },
    { level: 0, water: 'empty', wrings: 18 },
    'the terminal bucket state remains canonical through save and reload',
  );

  c.bucket.level = 0.01;
  c.bucket.water = 'dirty';
  ensureCleaningToolState(state);
  assert.equal(c.bucket.level, 0, 'legacy unusable residue heals to zero');
  assert.equal(c.bucket.water, 'empty', 'legacy unusable residue cannot remain visibly dirty');
});
