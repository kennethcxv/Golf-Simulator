// NO GAME ASSET SHIPS WITH A CAMERA OR A LIGHT BAKED INTO IT.
//
...
test('no GLB carries an exported camera or light', () => {
  const offenders = [];
  for (const f of files) {
    let rec;
    try { rec = auditGlb(f); } catch { continue; }
    if (rec.cameras > 0) offenders.push(`${rec.file}: ${rec.cameras} camera(s)`);
    if (rec.lights > 0) offenders.push(`${rec.file}: ${rec.lights} light(s)`);
  }
