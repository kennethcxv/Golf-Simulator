...
import { HOLE_STATUS, ZONE } from '../src/sim/constants.js';
...
test('generateMarketplace produces a full roster of valid property records', () => {
  ...
  for (const p of props) {
    ...
    assert.ok(p.size === 3 || p.size === 9 || p.size === 18, `size 3, 9, or 18, got ${p.size}`);
  ...
  const startingProperty = props.find((property) => property.id === STARTING_PROPERTY_ID);
  assert.equal(startingProperty?.name, STARTING_PROPERTY_NAME, 'stable ID uses the Pine Hills display name');
  assert.equal(startingProperty?.size, 3, 'Pine Hills is the compact three-hole onboarding property');
});
...
test('Pine Hills starts with deterministic bare wear while protected golf surfaces remain intact', () => {
  const props = generateMarketplace(4242);
  const starter = props.find((property) => property.id === STARTING_PROPERTY_ID);
  const course = buildPropertyCourse(starter);
  const dirtCells = Array.from(course.zones).filter((zone) => zone === ZONE.DIRT).length;
  const paintedDirt = Array.from(course.paint).filter((zone) => zone === ZONE.DIRT).length;

  assert.ok(dirtCells >= 12 && dirtCells <= 80, `restrained starter wear covers ${dirtCells} cells`);
  assert.ok(paintedDirt >= dirtCells, 'authored paint retains dirt hidden beneath protected vectors');
  for (const hole of course.holes) {
    assert.equal(course.zones[hole.tee.y * course.w + hole.tee.x], ZONE.TEE, `${hole.name} tee remains turf`);
    assert.equal(course.zones[hole.pin.y * course.w + hole.pin.x], ZONE.GREEN, `${hole.name} pin remains green`);
  }

  const rebuilt = buildPropertyCourse(starter);
  assert.deepEqual(Array.from(rebuilt.paint), Array.from(course.paint), 'starter wear is deterministic');
});
...
test('every listed property builds a real, playable course matching its record', () => {
  ...
  for (const p of props) {
    ...
    if (p.size === 3) assert.ok(yd >= 700 && yd <= 1400, `${p.name}: 3-hole yardage ${Math.round(yd)}`);
    else if (p.size === 9) assert.ok(yd >= 1500 && yd <= 3800, `${p.name}: 9-hole yardage ${Math.round(yd)}`);
