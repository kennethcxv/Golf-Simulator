...
test('tee-time payment revenue cannot replay after save/load', () => {
  const state = newGame('relaxed', 911);
  const booking = bookSlot(state, 0, 7 * 60, {
    holder: 'Saved Booker', paymentPlan: 'prepaid', paymentMethod: 'card', cardOnFile: true,
  }).res;
  const financeEntry = state.reservations.financeEntries.find((entry) => entry.reservationId === booking.id);
  assert.ok(financeEntry?.transactionId);
  const cash = state.cash;
  const loaded = deserialize(serialize(state));
  const replay = completeReservationPayment(loaded, booking.id, { transactionId: financeEntry.transactionId });
  assert.equal(replay.ok, true);
  assert.equal(replay.idempotent, true);
  assert.equal(loaded.cash, cash);
  assert.equal(loaded.ledger.entries.filter((entry) => entry.idempotencyKey === `golf-operations:${financeEntry.id}`).length, 1);
});
