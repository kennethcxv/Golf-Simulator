...
import {
  COUNTER, COUNTER_TOP, FIXTURES, INTERIOR, REGISTER, frontDeskPoint,
} from '../src/data/shopLayout.js';
import {
  FRONT_DESK_RUNTIME_ASSET_NUMBERS, FURNITURE_ASSET_NUMBERS, PLACEABLES, PRODUCT_PLACEMENT_POLICY,
...
test('the unified catalog covers existing fixtures, Assets 61-100, and every placement contract field', () => {
  ...
  for (const number of FRONT_DESK_RUNTIME_ASSET_NUMBERS) {
    const meta = placeableById(`asset-${String(number).padStart(3, '0')}`);
    assert.equal(meta.render.owner, 'unified-61-100-runtime', `asset ${number} has one render owner`);
    assert.equal(meta.storageBehavior, 'recovery-only');
    assert.ok(meta.placementRestrictions.includes('authored-socket-only'));
  }
...
test('floor placement supports small props and commercial-scale furniture without floating or sinking', () => {
  ...
  const state = emptyMovables();
  const floating = validate(state, 'asset-068', { ...floor(-2, -2), y: 0.2 });
...
test('counter and shelf placement uses finite authored surfaces and protects checkout equipment', () => {
  ...
  assert.equal(validate(state, 'asset-076', counterProp).ok, true);
  const blocked = validate(state, 'asset-076', {
    ...counterProp, x: REGISTER.monitor.x, z: REGISTER.monitor.z,
  });
  assert.ok(blocked.codes.includes('protected-equipment'));
  const ballShelf = placementSurfaces(state)
    .find((surface) => surface.id === 'surface:fixture:shelf_balls:shelf-2');
  const spray = {
    x: ballShelf.x + 0.3, y: ballShelf.y, z: ballShelf.z, ry: ballShelf.ry, surface: 'shelf',
    attachment: { parentId: ballShelf.id }, room: 'sales',
  };
  assert.equal(validate(state, 'asset-076', spray).ok, true);
  assert.ok(validate(state, 'asset-076', { ...spray, x: ballShelf.x - 2 }).codes.includes('surface-bounds'));
});
...
test('collision rejects furniture, walls, player clearance, protected work areas, and every door route', () => {
  ...
  assert.equal(commitObjectPlacement(state, 'asset-082', floor(-4, -2), exactOptions).ok, true);
  assert.ok(validate(state, 'asset-068', floor(-4, -2)).codes.includes('object-overlap'));
  assert.ok(validate(state, 'asset-068', floor(-INTERIOR.w, 0)).codes.includes('wall-collision'));
  const actorBlocked = validateObjectPlacement(state, 'asset-068', floor(-2, -2), {
    ...exactOptions, actorPosition: { x: -2, z: -2 },
  });
  assert.ok(actorBlocked.codes.includes('player-clearance'));
  const mainDoor = validate(state, 'asset-068', floor(-0.8, 5.6));
  assert.ok(mainDoor.codes.some((code) => code.includes('main-door') || code.includes('door-swing-main')));
  const receiving = validate(state, 'asset-068', floor(9.4, -3.6));
  assert.ok(receiving.codes.some((code) => code.includes('receiving-door') || code.includes('door-swing-back')));
  const stockDoor = validate(state, 'asset-068', floor(8.9, 2.0));
  assert.ok(stockDoor.codes.some((code) => code.includes('stockroom-access') || code.includes('door-swing-stock')));
  const checkout = validate(state, 'asset-068', floor(COUNTER.staffStand.x, COUNTER.staffStand.z));
...
test('physically touching footprints are allowed while true overlap is rejected', () => {
  ...
  const touching = floor(-3 + width / 2 + placeableById('asset-068').bounds.width / 2, -2);
  const result = validate(state, 'asset-068', touching);
...
test('save/load preserves wall normals, surface attachments, variants, stored/sold state, styles, and required relationships', () => {
  ...
  assert.equal(commitObjectPlacement(state, 'asset-084', {
    x: top.x, y: top.y, z: top.z, ry: 0, surface: 'counter',
    attachment: { parentId: top.id, socket: 'SOCKET_PLACEMENT' }, room: 'sales',
...
test('empty and heavily furnished clubhouses preserve every required customer route', () => {
  ...
  const floorAssets = PLACEABLES.filter((meta) => (
    meta.render?.kind === 'glb'
    && meta.source === 'assets-51-100-project-owned'
    && meta.surfaceRules.allowed.includes('floor')
    && !meta.requiredObject
  ));
