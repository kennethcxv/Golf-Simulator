...
test('the customer choice is persistent, starts automatically, and never exposes manual payment buttons', () => {
  assert.doesNotMatch(registerSource, /\(key === 't' \|\| key === 'T'\)/);
  assert.match(registerSource, /paymentAutoTimer\s*=\s*AUTO_PAYMENT_HOLD/);
  assert.match(registerSource, /paymentAutoTimer === 0\) choosePayment\(preferredPayment\(\)\)/);
