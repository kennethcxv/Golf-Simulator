...
test('the allowlist has stable IDs and exact authored support contracts', () => {
  ...
  assert.deepEqual(floor.bounds, { minX: -10.25, maxX: 10.25, minZ: -6.5, maxZ: 6.5 });
  ...
  assert.deepEqual(counter.worldPose, { x: 3.8000000000000003, y: 1.01, z: 6.15, ry: 0 });
  assert.deepEqual(counter.bounds, { minX: -0.95, maxX: 0.95, minZ: -0.23, maxZ: 0.23 });

  const centres = [
    [11.65, 0.62], [13, 0.62], [14.35, 0.62], [12.325, -0.67], [13.675, -0.67],
  ];
...
test('floor validation covers geometry, fixtures, door clearways, staff space, and other boxes', () => {
  const state = fresh();
  const open = previewBoxPlacement(state, carton(1), worldTarget(FLOOR_BOX_SURFACE_ID, 0, 2));
  ...
  const fixture = previewBoxPlacement(state, carton(1), worldTarget(FLOOR_BOX_SURFACE_ID, -6.9, -6.15));
  ...
  const staff = previewBoxPlacement(state, carton(1), worldTarget(FLOOR_BOX_SURFACE_ID, 2.9, 5.35));
  assert.equal(staff.code, BOX_PLACEMENT_CODES.STAFF_WORKSPACE);

  state.shop.deliveries.boxes.push(carton(2, {
    loc: 'world', x: 0, z: 2, ry: Math.PI / 4,
  }));
  const overlap = previewBoxPlacement(state, carton(3), worldTarget(FLOOR_BOX_SURFACE_ID, 0.1, 2.0));
...
test('capabilities derive from the saved box surface without parallel state', () => {
  ...
  assert.equal(boxPlacementCapabilities(state, carton(5, {
    loc: 'world', surfaceId: FLOOR_BOX_SURFACE_ID, x: 7.2, z: -5.3, ry: 0,
