...
test('strict checkout acceptance derives its cashier stand from the live clubhouse transform', () => {
  ...
  assert.match(source, /const origin = clubhouse\.interior\.position;/u);
  assert.match(source, /const \{ REGISTER: liveRegister \} = await import/u);
  assert.match(source, /walk\.x = liveRegister\.stand\.x \+ origin\.x;/u);
  assert.match(source, /walk\.z = liveRegister\.stand\.z \+ origin\.z;/u);
