...
test('Fairway Spring and Bunker Bites are coherent tier-one retail provisions', () => {
  ...
  const fixtureBySku = { water1: 'cold_drinks', snack1: 'snackrack' };
  for (const sku of [water, snack]) {
    ...
    assert.deepEqual(packaging.allowedStocking.fixtureIds, [fixtureBySku[sku.id]]);
  }
});
...
test('the opening cooler and snack rack each own their lines and every retail SKU has one fixture', () => {
...
test('provisions capacity follows the cooler sockets and separate snack facings', () => {
  const authoredWater = OPENING_DRINKS_COOLER_SOCKETS.filter((socket) => socket.skuId === 'water1');
  const water = slotsFor('water1');
  const snack = slotsFor('snack1');

  assert.deepEqual(water.map((slot) => slot.socketName), authoredWater.map((slot) => slot.name));
  assert.equal(water.length, OPENING_DRINKS_COOLER_CAPACITY.bySku.water1);
  for (let index = 0; index < water.length; index += 1) {
    assert.ok(Math.abs(water[index].x - authoredWater[index].gltfPositionMeters.x / 0.9144) < 1e-9);
    assert.ok(Math.abs(water[index].y - authoredWater[index].gltfPositionMeters.y / 0.9144) < 1e-9);
  }
  ...
  assert.equal(homeFixture('snack1').id, 'snackrack');
});
...
test('provisions stock only their authored fixture, respect capacity, and retain leftovers', () => {
  ...
  const snackState = newGame('relaxed', 1804);
  snackState.shop.inventory.snack1.back = 12;
  assert.equal(takeFromBack(snackState, 'snack1').taken, 12);
  const snackFull = stockFixture(snackState, 'snackrack', 99);
  assert.deepEqual(
  ...
  assert.equal(
    snackState.shop.inventory.snack1.shelf
      + snackState.shop.inventory.snack1.back
      + carriedGoods(snackState).qty,
    12,
  );
});
...
test('provisions orders use the live Fairway ETA and exact twelve-unit carton layouts', () => {
  ...
  for (const [id, layoutId] of Object.entries(expectations)) {
    ...
    assert.ok(result.order.deliveryMin > state.clock.minutes);
