...
import {
  ...
  presentCard, insertCard, submitCardAmount, enterCardDigit, totalOf,
  runCard, retryCard, cancelCard,
...
test('an explicitly declined card does not end the sale - the customer tries a second one', () => {
  ...
  const bad = runCard(tx, { force: 'declined' });
...
test('the drawer starts with a real float - you can always make change', () => {
  ...
  assert.equal(drawer[0.2], undefined, 'and no 20-unit coin, which this currency does not mint');
});
