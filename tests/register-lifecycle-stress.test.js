...
test('120 alternating card/cash sales settle once, reset cleanly, and keep persistent structures bounded', () => {
  ...
  const supplied = must(submitPurchaseOrders(state, {
    lines: skuIds.map((skuId) => ({ skuId, quantity: SALE_COUNT })),
    idempotencyKey: 'register-lifecycle-stress:opening-stock',
  }), 'purchase stress stock');
  for (const skuId of skuIds) {
    const inv = state.shop.inventory[skuId];
    const openingShelf = inv.shelf;
    if (openingShelf > 0) {
      must(moveInventory(state, {
        from: INVENTORY_STAGE.SHELF,
        to: INVENTORY_STAGE.RESERVE,
        skuId,
        quantity: openingShelf,
        reason: 'Stress scenario clears the opening display before restocking',
      }), `move opening ${skuId} to reserve`);
      inv.shelf -= openingShelf;
      inv.back += openingShelf;
    }
    const order = supplied.orders.find((candidate) => (
      candidate.lines.some((line) => line.skuId === skuId)
    ));
    const line = order?.lines.find((candidate) => candidate.skuId === skuId);
    assert.ok(order && line, `${skuId} stress stock has purchase provenance`);
    must(moveInventory(state, {
      ...
      lineId: line.id,
      quantity: SALE_COUNT,
      reason: 'Stress scenario reserve setup',
    }), `receive stress ${skuId} into reserve`);
    inv.back += SALE_COUNT;
  }
  assert.equal(reconcileInventory(state).ok, true, 'all stress stock reconciles before the first restock');
  for (const skuId of skuIds) {
