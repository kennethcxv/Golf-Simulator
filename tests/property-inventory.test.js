...
import { placeDecor, placeOrder, cancelOrder, removeDecor } from '../src/sim/shop.js';
...
test('the production placeable catalog separates durable property assets from retail stock', () => {
  assert.equal(PLACEABLE_ITEM_CATALOG.length, 6);
  for (const spec of PLACEABLE_ITEM_CATALOG) {
    assert.match(spec.assetId, /^shop-decor:/);
    ...
    assert.deepEqual(spec.variants, ['standard']);
